...
from typing import List, DefaultDict, Any, Optional, Tuple
from interview_details_agent.base import BaseInterviewConfiguration ,InterviewRoundData, InterviewTopicData, InterviewRoundDetails
...
class SubTopicNode(BaseModel):
    ...
    sections: DefaultDict[str, bool] = Field(default_factory=dict, description="Section completion status")
...
class InterviewRoundDataGraph(BaseModel):
    ...
    def create_topic_node(self, topic_data:InterviewTopicData):
        ...
        for subtopic in subtopic_list:
            ...
            subtopic_node = SubTopicNode(subtopic_data = subtopic_data, sections = {section: False for section in subtopic_data.sections})
...
class InterviewTopicTracker(BaseModel):
    ...
    def get_topic_subtopic_for_discussion(self, interview_round: InterviewRound) -> Tuple[Optional[InterviewTopicData], Optional[SubTopicData], Optional[str], bool]:
        ...
        try:
            ...
            if interview_round == InterviewRound.ROUND_ONE:
                topic_node = self.interview_round_data_graph.get_next_topic_node(InterviewRound.ROUND_ONE)
                if topic_node is None:
                    interview_round = InterviewRound.ROUND_TWO
                    is_interview_round_changed = True
                else:
                    topic_data = topic_node.topic_data
                    subtopic_node = self.interview_round_data_graph.get_next_subtopic_node(topic_node)
                    if subtopic_node:
            ...
            if interview_round == InterviewRound.ROUND_TWO:
                ...
                next_topic_node = self.interview_round_data_graph.get_next_topic_node(InterviewRound.ROUND_TWO)
                if next_topic_node:
                    topic_data = next_topic_node.topic_data
                    next_subtopic_node = self.interview_round_data_graph.get_next_subtopic_node(next_topic_node)
                    if next_subtopic_node:
                        section = self.interview_round_data_graph.get_next_section(next_subtopic_node)
    ...
    def get_last_completed_topic_name(self, interview_round):
        last_topic_node = self.interview_round_data_graph.get_last_completed_topic_node(interview_round)
    ...
    def get_topic_data_based_on_name(self, interview_round, topic_name):
        topic_node = self.interview_round_data_graph.get_topic_node_by_name(interview_round, topic_name)
    ...
    def get_last_completed_subtopic_name(self, interview_round, topic_name):
        last_subtopic_node = self.interview_round_data_graph.get_last_completed_subtopic_node(interview_round, topic_name)
    ...
    def get_subtopic_nodes(self, interview_round, topic_name):
        for topic_node in self.interview_round_data_graph.interview_round_topic_data_graph[interview_round]:
            if topic_node.get_topic_name() == topic_name:
                return topic_node.subtopic_nodes
        return None
                
    def update_topic_completion_status(self, interview_round, topic_name, subtopic_name, section_name):
        self.interview_round_data_graph.update_subtopic_status(interview_round, topic_name, subtopic_name, section_name)
    ...
    def get_all_uncompleted_subtopics(self, interview_round, topic_name):
        return self.interview_round_data_graph.get_uncompleted_subtopics(interview_round, topic_name)
    ...
    def get_conversation_history_for_subtopic(self, current_interview_round, topic_name, subtopic_name):
        return self.memory_graph.get_subtopic_conversation_memory(current_interview_round, topic_name, subtopic_name)
    ...
    def get_conversation_history_for_all_topics(self, current_interview_round):
        ...
        topic_names = [node.get_topic_name() for node in 
                      self.interview_round_data_graph.interview_round_topic_data_graph[current_interview_round]]
        
        # Batch fetch from memory graph
        conversation_history = []
        for topic_name in topic_names:
            history = self.memory_graph.get_topic_conversation_memory(current_interview_round, topic_name)
            if history:
    ...
    def get_topic_summary_of_all_completed_topics(self, current_interview_round):
        ...
        completed_topics = []
        for topic_node in self.interview_round_data_graph.interview_round_topic_data_graph[current_interview_round]:
        ...
        for topic_name in completed_topics:
            topic_summary.extend(self.memory_graph.get_topic_summary(current_interview_round, topic_name))
    ...
    def get_subtopic_summary(self, current_interview_round, topic_name, subtopic_name):
        return self.memory_graph.get_subtopic_summary(current_interview_round, topic_name, subtopic_name)
    
    def get_all_conversation_summary_for_topic(self, current_interview_round, topic_name):
        return self.memory_graph.get_all_subtopics_conversation_summary(current_interview_round, topic_name)

    # we have three add functions for memory graph
    def add_dialog_to_memory(self, current_interview_round, topic_name, subtopic_name, item:MasterChatMessage) -> bool:
        return self.memory_graph.add_dialog_to_memory(current_interview_round, topic_name, subtopic_name, item)

    def add_subtopic_summary_to_memory(self, current_interview_round, topic_name, subtopic_name, item:List[str]):
        self.memory_graph.add_subtopic_summary_to_memory(current_interview_round, topic_name, subtopic_name, item)
    ...
    def update_multiple_sections(self, interview_round: InterviewRound, topic_name: str, 
                               subtopic_name: str, section_names: List[str]) -> None:
    ...
    def get_completion_statistics(self, interview_round: InterviewRound) -> dict[str, Any]:
        ...
        topics = self.interview_round_data_graph.interview_round_topic_data_graph.get(interview_round, [])
        ...
        if stats['total_topics'] > 0:
            stats['topic_completion_percentage'] = int((stats['completed_topics'] / stats['total_topics']) * 100)
        if stats['total_subtopics'] > 0:
            stats['subtopic_completion_percentage'] = int((stats['completed_subtopics'] / stats['total_subtopics']) * 100)
