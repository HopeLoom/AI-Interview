...
class SQLiteAdapter(DatabaseInterface):
            ...
            async with self._get_connection() as conn:
                ...
                if result:
                    ...
                    self.user_data = UserProfile(
                        ...
                        panelist_profiles=json.loads(row_dict["panelist_profiles"])
                        if row_dict["panelist_profiles"]
                        else None,
                        panelist_images=json.loads(row_dict["panelist_images"])
                        if row_dict["panelist_images"]
                        else None,
                        role=row_dict["role"],
    ...
    async def update_session(self, user_id: str, session_id: str, updates: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            query = (
                f"UPDATE sessions SET {', '.join(set_clauses)} WHERE user_id = ? AND session_id = ?"
            )
        ...
        try:
            async with self._get_connection() as conn:
                ...
                result["interview_transcript"] = {
                    str(i): {"speaker": t[0], "dialog": t[1]} for i, t in enumerate(transcripts)
                }
    ...
    async def delete_simulation_config(self, config_id: str) -> bool:
        ...
        try:
            async with self._get_connection() as conn:
                await conn.execute(
                    "DELETE FROM simulation_configs WHERE config_id = ?", (config_id,)
                )
                await conn.commit()
