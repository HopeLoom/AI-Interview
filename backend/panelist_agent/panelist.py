...
import os
import json
from panelist_agent.base import BasePanelist, BasePanelistConfiguration
from core.resource.model_providers.schema import ChatMessage, AssistantChatMessage, ChatModelProvider, ReflectionChatMessage, ReasoningChatMessage
...
from panelist_agent.base import ResponseWithReasoningOutputMessage, ResponseOutputMessage, DomainKnowledgeOutputMessage, PromptInput, ReflectionOutputMessage, EvaluationOutputMessage, ReasoningOutputMessage
from master_agent.base import InterviewRound, WebSocketMessageTypeFromClient, SystemMessageType, SystemMessageStructure
from master_agent.base import MasterMessageStructure, SlaveMessageStructure, CommunicationMessage, SimulationRole
from master_agent.base import TopicSectionCompletionOutputMessage, SpeakerDeterminationOutputMessage
...
class Panelist(BasePanelist):
    ...
    config: BasePanelistConfiguration = BasePanelistConfiguration(name="Panelist", description="Panelist")
    ...
    def __init__(self, 
        ...
        self.prompt_strategy:PanelistPromptStrategy = PanelistPromptStrategy(panelist_config,
                                                                             interview_config,
                                                                             database)
        ...
        for file_pattern in ["_respond.json", "_think.json", "_reflection.json", "_domain_knowledge.json", "_eval.json"]:
    ...
    def get_info_of_other_panelists(self, npc_profile_list:List[Profile]):
        filtered_list = [profile for profile in npc_profile_list if profile.background.name != self.name]
    ...
    def update_reflection_history(self, user_input:ReflectionOutputMessage):
        message = ReflectionChatMessage(reflection = user_input.reflection, 
    ...
    def update_reasoning_history(self, user_input:ReasoningOutputMessage):
        message = ReasoningChatMessage(interview_thoughts_for_myself = user_input.interview_thoughts_for_myself)
    ...
    def parse_process_reason_model(self, response:AssistantChatMessage, prompt:ChatMessage):
        output:ReasoningOutputMessage = self.prompt_strategy.parse_response_reason_content(response)
        return output
    ...
    def parse_process_reflect_model(self, response: AssistantChatMessage, prompt: ChatMessage):
        output:ReflectionOutputMessage = self.prompt_strategy.parse_response_reflect_content(response)
        return output
    
    def parse_process_evaluate_model(self, response:AssistantChatMessage, prompt:ChatMessage):
        output:EvaluationOutputMessage = self.prompt_strategy.parse_response_evaluate_content(response)
        return output
    
    def parse_process_domain_knowledge_model(self, response: AssistantChatMessage, prompt: ChatMessage):
        output:DomainKnowledgeOutputMessage = self.prompt_strategy.parse_response_domain_knowledge_content(response)
        return output

    def parse_process_respond_with_reasoning_model(self, response: AssistantChatMessage, prompt: ChatMessage):
        output:ResponseWithReasoningOutputMessage = self.prompt_strategy.parse_process_respond_with_reasoning_model(response)
        return output
    ...
    async def respond(self, input_message: CommunicationMessage, 
                      output_reason:ReasoningOutputMessage, 
                      activity_progress,
                      domain_knowledge_output:DomainKnowledgeOutputMessage):
        
        prompt_input = PromptInput(response_type = PanelistPromptStrategy.RESPONSE_TYPE.RESPOND,
                                   reason = output_reason,
                                    activity_progress = activity_progress,
                                    domain_knowledge = domain_knowledge_output,
                                    candidate_profile = self.master_instance.get_candidate_profile() if self.master_instance is not None else Profile(),
                                    message = input_message)

        prompt:ChatPrompt = super().build_prompt(prompt_input)
        token_count  = self.llm_provider.count_message_tokens(prompt.messages, self.config.settings.gpt_4o) 
    ...
    async def get_domain_knowledge(self, input_message:CommunicationMessage, 
                                   output_reason:ReasoningOutputMessage,
                                   activity_progress):
                
        prompt_input = PromptInput(response_type = PanelistPromptStrategy.RESPONSE_TYPE.DOMAIN_KNOWLEDGE,
                                    reason = output_reason,
                                    activity_progress = activity_progress,
                                   message = input_message)
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, f"panelist_domain_knowledge_{self.name}", output.model_dump() if hasattr(output, 'model_dump') else output)
    ...
    async def think(self, input_message: CommunicationMessage, activity_progress):
        ...
        prompt_input = PromptInput(response_type = PanelistPromptStrategy.RESPONSE_TYPE.REASON,
                                   activity_progress = activity_progress,
                                   candidate_profile = self.master_instance.get_candidate_profile() if self.master_instance is not None else Profile(),
                                   message = input_message)
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, f"panelist_reasoning_{self.name}", output.model_dump() if hasattr(output, 'model_dump') else output)
    ...
    async def respond_with_reasoning(self, input_message:CommunicationMessage, 
                                     activity_progress):
        
        prompt_input = PromptInput(response_type = PanelistPromptStrategy.RESPONSE_TYPE.RESPOND_WITH_REASONING,
                                   activity_progress = activity_progress,
                                   candidate_profile = self.master_instance.get_candidate_profile() if self.master_instance is not None else Profile(),
                                   message = input_message
                                )
    ...
    async def react(self, communicationMessage: CommunicationMessage):
        ...
        response_output_with_reasoning = await self.respond_with_reasoning(communicationMessage, activity_progress)
        self.logger.info(f"Dialog output: {response_output_with_reasoning}")
        dialog = response_output_with_reasoning.response
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, f"panelist_response_reasoning_{self.name}", response_output_with_reasoning.model_dump() if hasattr(response_output_with_reasoning, 'model_dump') else response_output_with_reasoning)
        self.update_conversation_history(dialog)
       
        response = CommunicationMessage.message_to_master(sender = SimulationRole.PANELIST, 
    ...
    async def evaluate(self, topic_name, 
                       subtopic_name, 
                       topic_data, 
                       subtopic_data,
                       topic_conversation, 
        ...
        prompt_input = PromptInput(response_type = PanelistPromptStrategy.RESPONSE_TYPE.EVALUATE,
                                   message = message,
                                   ...
                                   activity_progress = self.activity_instance.get_recent_progress() if self.activity_instance is not None else "")
        
        prompt:ChatPrompt = super().build_prompt(prompt_input)
        output:EvaluationOutputMessage = await super().evaluate(prompt)
        
        self.evaluation_output_message = output
        self.evaluation_output_messages_topic_subtopic_mapping[topic_name + "," + subtopic_name] = output
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, f"panelist_evaluation_{self.name}", self.evaluation_output_message.model_dump() if hasattr(self.evaluation_output_message, 'model_dump') else self.evaluation_output_message)
    ...
    async def process_system_message(self, communicationMessage:CommunicationMessage):
        system_message: SystemMessageStructure = cast(SystemMessageStructure, communicationMessage.content)
    ...
    async def run(self):
        ...
        while True:
            try:
                communicationMessage: CommunicationMessage = self.receiving_message_queue.get_nowait()
