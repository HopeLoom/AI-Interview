...
from .constants import AVAILABLE_JOB_TYPES
from .models import (
...
class InterviewConfigurationService:
    ...
    def get_example_starter_code(
        ...
        for example in examples:
            ...
            code_path = (
                root_path / "onboarding_data" / "examples" / job_type / example.activity_code_path
            )
            with open(code_path) as f:
                code = f.read()
            starter_code.append(
                StarterCodeData(code=code, description=example.activity_details.scenario)
            )
        return starter_code
    ...
    async def generate_full_configuration(
        self, config_input: FrontendConfigurationInput, company_id: str, job_type: str
    ) -> ConfigurationGenerationResponse:
        """
        ...
        try:
            main_logger.info(
                f"Starting full configuration generation for company: {company_id}, job: {job_type}"
            )
            ...
            job_details: JobDetails | None = await self._generate_job_details(
                config_input, company_id, job_type
            )
            ...
            activity_details = await interview_agent.generate_activity_details(
                example_job_details, example_activity_details
            )
            ...
            generated_starter_code: StarterCodeData = await interview_agent.generate_starter_code(
                example_activity_details, example_starter_code, activity_details
            )
            ...
            character_data_output = await interview_agent.generate_character_information(
            ...
            for character_data in character_data_list:
                ...
                if panelist_profile:
                    ...
                    main_logger.info(
                        f"Uploaded panelist profile for {character_data.character_name} to: {profile_url}"
                    )
                    ...
                    try:
                        image_url = await self._generate_panelist_image(
                            character_data, panelist_profile
                        )
                        if image_url:
                            panelist_images_urls.append(image_url)
                            main_logger.info(
                                f"Generated and uploaded image for {character_data.character_name} to: {image_url}"
                            )
                    except Exception as e:
                        main_logger.warning(
                            f"Failed to generate image for {character_data.character_name}: {e}"
                        )
                else:
                    main_logger.warning(
                        f"Failed to generate profile for character {character_data.character_name}"
                    )
            ...
            main_logger.info(
                f"Successfully generated {len(panelist_profiles_urls)} panelist profiles and {len(panelist_images_urls)} images"
            )
            ...
            await self._store_configuration(
                config_id, invitation_code, master_config, company_id, config_input
            )

            response.simulation_config = interview_config.model_dump()
            main_logger.info(
                "Step 6 completed: Characters generated and simulation config prepared"
            )
            ...
            main_logger.info(
                f"Processing {len(resume_file_ids)} resume files for company {company_id}, job {job_type}"
            )
            ...
            simulation_config_url = await self._upload_json_to_storage(
                interview_config.model_dump(), company_id, f"{job_type}_simulation_config.json"
            )
            ...
            try:
                ...
                starter_code_url = await self._upload_file_to_storage(
                    "starter_code.txt", company_id, "starter_code.txt"
                )
            ...
            for resume_file_id in resume_file_ids:
                try:
                    resume_file_path = os.path.join(
                        f"static/{company_id}/{job_type}/resume", resume_file_id
                    )
                    ...
                    if candidate_profile is None:
                        main_logger.warning(
                            f"Failed to generate candidate profile for: {resume_file_id}"
                        )
                        failed_candidates += 1
                        continue
                    ...
                    candidate_email = (
                        getattr(candidate_profile, "email", None)
                        or f"candidate_{uuid.uuid4().hex[:8]}@example.com"
                    )
                    ...
                    try:
                        resume_url = await self._upload_file_to_storage(
                    ...
                    try:
                        candidate_profile_url = await self._upload_json_to_storage(
                            candidate_profile_data, user_id, "candidate_profile.json"
                        )
                        main_logger.info(
                            f"Uploaded candidate profile for {candidate_name} to: {candidate_profile_url}"
                        )
                    except Exception as e:
                        main_logger.warning(
                            f"Failed to upload candidate profile for {candidate_name}: {e}"
                        )
                    ...
                    try:
                        await self._store_candidate_data(
                            user_id=user_id,
                            candidate_info={"name": candidate_name, "email": candidate_email},
                        ...
                        main_logger.info(
                            f"Successfully processed candidate: {candidate_name} ({candidate_email})"
                        )
                        processed_candidates += 1
                    except Exception as e:
                        main_logger.warning(
                            f"Failed to store candidate data in Firestore for {candidate_name}: {e}"
                        )
            ...
            main_logger.info(
                f"Configuration generation completed. Processed: {processed_candidates}, Failed: {failed_candidates}"
            )

            if processed_candidates > 0:
                main_logger.info(
                    f"Successfully generated configuration for {processed_candidates} candidates"
                )
    ...
    def _validate_configuration_input(
        self, config_input: FrontendConfigurationInput
    ) -> ConfigurationValidationResult:
        ...
        if (
            not config_input.job_details.job_description
            and config_input.job_details.input_type == "text"
        ):
            result.errors.append("Job description is required when input type is text")

        if (
            config_input.job_details.input_type == "pdf"
            and not config_input.job_details.job_file_id
        ):
        ...
        if hasattr(config_input, "character_request") and config_input.character_request:
            if (
                hasattr(config_input.character_request, "character_count")
                and config_input.character_request.character_count <= 0
            ):
                result.errors.append("Character count must be greater than 0")
            if (
                hasattr(config_input.character_request, "roles")
                and not config_input.character_request.roles
            ):
        ...
        if result.is_valid:
            ...
            main_logger.error(
                f"Configuration input validation failed with {len(result.errors)} errors: {result.errors}"
            )

        return result

    async def _generate_job_details(
        self, config_input: FrontendConfigurationInput, company_id: str, job_name: str
    ) -> JobDetails | None:
        ...
        try:
            ...
            frontend_job_title = (
                frontend_job_info.job_title if frontend_job_info.job_title else None
            )
            job_title_instruction = (
                f"Use the job title: '{frontend_job_title}'"
                if frontend_job_title
                else "Extract the job title from the description"
            )
            ...
            from core.prompting.schema import ChatMessage, ChatPrompt

            chat_prompt = ChatPrompt(messages=[ChatMessage(role="user", content=prompt)])

            response = await self.llm_provider.create_chat_completion(
                chat_messages=chat_prompt, model_name="gpt-4o", temperature=0.7, is_json_mode=True
            )
            ...
            try:
                ...
                main_logger.info(
                    f"Successfully generated job details for: {job_details.job_title if hasattr(job_details, 'job_title') else 'Unknown'}"
                )
                return job_details
    ...
    async def _generate_panelist_profile(self, character_data) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            background = await background_generator.generate_background_info(
                self.llm_provider, character_data
            )
            personality = await personality_generator.generate_personality_info(
                self.llm_provider, background, character_data
            )
            ...
            main_logger.error(
                f"Error generating panelist profile for {character_data.character_name}: {e}"
            )
            return None
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                if image_path and os.path.exists(image_path):
                    ...
                    image_url = await self._upload_file_to_storage(
                        image_path, "temp_user", filename
                    )
                    main_logger.info(
                        f"Successfully generated and uploaded image for {character_data.character_name}"
                    )
                    return image_url
                else:
                    main_logger.warning(
                        f"Failed to generate image for {character_data.character_name}"
                    )
                    return None

        except Exception as e:
            main_logger.error(
                f"Error generating panelist image for {character_data.character_name}: {e}"
            )
            return None
    ...
    async def _upload_file_to_storage(
        self, file_path: str, user_id: str, file_name: str
    ) -> str | None:
        """
    ...
    async def _store_candidate_data(
        ...
        try:
            ...
            main_logger.error(
                f"Error storing candidate data for {candidate_info.get('name', 'Unknown')}: {e}"
            )
    ...
    async def _load_template_config(self, job_type: str) -> Optional[BaseInterviewConfiguration]:
        ...
        try:
            ...
            root_path = Path(__file__).parent.parent.parent
            template_path = (
                root_path / "onboarding_data" / "templates" / job_type / "simulation_config.json"
            )
    ...
    async def _generate_candidate_profile(self, resume_content: str):
        ...
        try:
            ...
            main_logger.info("Successfully generated candidate profile")
            return profile
        except Exception as e:
    ...
    async def _generate_question_and_code(
        ...
        try:
            ...
            activity_details = await interview_agent.run_activity_details(
                example_job_details, example_activity_output
            )

            # Generate starter code
            example_starter_code = StarterCodeData()
            example_starter_code.description = (
                f"Starter code for {request.programming_language} problem"
            )

            starter_code = await interview_agent.run_starter_code(
                example_activity_output, example_starter_code, activity_details
            )
    ...
    def _assemble_master_config(
        ...
        master_config.simulation_name = (
            f"interview_{interview_config.job_details.job_title.lower().replace(' ', '_')}"
        )
    ...
    async def _store_configuration(
        ...
        try:
            ...
            print(
                f"Configuration {config_id} with invitation code {invitation_code} stored for user {user_id}"
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    main_logger.warning(
                        "PDF reading libraries not available. Install PyPDF2 or pdfplumber"
                    )
