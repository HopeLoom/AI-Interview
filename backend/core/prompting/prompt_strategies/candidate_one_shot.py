...
from panelist_agent.base import PromptInput, DomainKnowledgeOutputMessage, ReasoningOutputMessage, ResponseOutputMessage
from master_agent.base import CommunicationMessage, MasterMessageStructure, SubTopicData, InterviewTopicData, InterviewRound
...
class CandidatePromptStrategy(BaseCandidatePromptStrategy):
    ...
    def convert_simulation_type(self, simulation_history: List[MasterChatMessage]):
        if simulation_history is None:
            return ""
        conversation_history_strings = [f"Speaker:{message.speaker}, dialog:{message.content}\n" for message in simulation_history]
    ...
    def _generate_domain_knowledge_prompt(self, input_message:PromptInput):
        ...
        if interview_round == InterviewRound.ROUND_TWO:
            panelist_data = [f"Name:{panelist.background.name}, Occupation:{panelist.background.current_occupation.occupation}" for panelist in panelist_list]
            panelist_data = "\n".join(panelist_data)
            panelist_prompt = (f"The other panelists in the interview are:{panelist_data}\n")
        else:
            panelist_prompt = "There are no other panelists in the interview and you are the only one\n" 
        ...
        if speaker_occupation.contains("engineer"):
            ...
            role_prompt = (
                "A HR manager is responsible for hiring, training, developing and retaining employees\n"
            )
        ...
        output_prompt = (
            f"Based on all the information you are provided with, you need to generate the domain knowledge in the following format{domain_output_message.model_dump_json()}.\n"
        )
