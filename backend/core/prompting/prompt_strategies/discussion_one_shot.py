...
class DiscussionPromptStrategy(BaseDiscussionPromptStrategy):
    ...
    def build_prompt(self, **kwargs):
        ...
        if response_type == DiscussionPromptStrategy.RESPONSE_TYPE.FEEDBACK_INFO:
            ...
            if isCode:
                user_prompt = (
                    f"Here are the code details:{activity_data.activity_code_generation}\n"
            ...
            user_prompt = (
                ...
                + job_details.model_dump_json()
                + "\n"
                + "Here are the interview details for which you should generate the activity: "
                + interview_details.model_dump_json()
                + "\n"
            )
            user_prompt += (
                "Here is the panelist involved in the interview: "
                + character_data.model_dump_json()
                + "\n"
            )
            user_prompt += (
                "Here is the overall feedback from the panelists for your previous activity generation: "
                + consensus_output.model_dump_json()
                + "\n"
            )
            user_prompt += (
                "Here is the previous activity details you generated: "
                + activity_data.activity_details.model_dump_json()
                + "\n"
            )
            ...
            user_prompt += (
                "Here is the overall feedback from the panelists for your previous code generation: "
                + consensus_output.model_dump_json()
                + "\n"
            )
            user_prompt += (
                "Here is the previous activity details you generated: "
                + activity_data.activity_code_generation.model_dump_json()
                + "\n"
            )
            ...
            user_prompt = (
                "Here are the feedback details: "
                + str(feedbackData)
                + "\n"
                + "Here are the activity details: "
                + activity_details.model_dump_json()
                + "\n"
            )
        ...
        prompt = ChatPrompt(
            messages=[ChatMessage.system(self.system_message), ChatMessage.user(user_prompt)]
        )
