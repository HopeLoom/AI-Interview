import chromadb
from core.resource.model_providers.schema import EmbeddingModelProvider, MasterChatMessage, EmbeddingModelResponse
...
class VectorDatabase:
    ...
    async def embed_text(self, text):
        response:EmbeddingModelResponse = await self.llm_provider.create_embedding(text=text, model_name="text-embedding-ada-002",
    ...
    def get_neighbours(self, center_index:int):
        window = 2
        neighbor_indices = list(range(center_index - window, center_index + window + 1))  # [14, 15, 16, 17, 18]

        results = self.collection.get(
            where={"turn_index": {"$in": neighbor_indices}},
            include=["documents", "metadatas"]
        )

        sorted_results = sorted(
            zip(results["metadatas"], results["documents"]),
            key=lambda x: x[0]["turn_index"]
        )
