...
class CandidateProfileGenerator:
    ...
    async def generate_candidate_profile(self, resume_data):
        ...
        personality = await generate_personality_info(
            self.llm_provider, user_background, resume_data
        )
        profile = Profile(
            background=user_background, personality=personality, character_id="Candidate"
        )
        return profile
