...
class VoiceBase:
    ...
    async def say_from_text(self, websocket_connection_manager, user_id: str, text: str, voice_name: str) -> SpeechResult:
        """Convert text to speech"""
        async with self._lock:
            try:
                result = await self._text_to_speech(websocket_connection_manager, user_id, text, voice_name)
                return SpeechResult(
                    user_id=user_id,
                    status=result.get("status", False),
                    error=result.get("error")
    ...
    async def _send_audio_chunk(self, websocket_connection_manager, user_id: str, audio_chunk: bytes) -> bool:
