...
from interview_details_agent.base import BaseInterview, BaseInterviewConfiguration, CharacterDataOutput, ActivityDetailsOutputMessage, StarterCodeData
...
from interview_details_agent.base import PromptInput, BaseInterviewPromptStrategy
...
class Interview(BaseInterview):
    ...
    def __init__(self, interview_id, interview_config:BaseInterviewConfiguration, llm_provider:ChatModelProvider):
    ...
    def parse_and_process_response_character_info(self, response: AssistantChatMessage, prompt: ChatMessage) -> CharacterDataOutput:
    ...
    def parse_and_process_response_activity_details_scenario(self, response: AssistantChatMessage, prompt: ChatMessage) -> ActivityDetailsOutputMessage:
        data = self.prompt_strategy.parse_response_content(response) 
        try:
            if response.content is None:
                raise Exception("Response content is None")
    ...
    def parse_and_process_response_starter_code(self, response: AssistantChatMessage, prompt: ChatMessage) -> StarterCodeData:
        data = self.prompt_strategy.parse_response_content(response) 
        try:
            if response.content is None:
                raise Exception("Response content is None")
    ...
    async def generate_activity_details(self, example_job_details, example_activity_details) -> ActivityDetailsOutputMessage:
        prompt_input = PromptInput()
    ...
    async def generate_starter_code(self, example_activity_details, example_starter_code_output, generated_activity_details_output) -> StarterCodeData:
        prompt_input = PromptInput()
        prompt_input.response_type = BaseInterviewPromptStrategy.RESPONSE_TYPE.STARTER_CODE_GENERATION
        prompt_input.job_details = self.job_details
        prompt_input.interview_round_details = self.interview_round_details
        prompt_input.example_activity_details_output = example_activity_details
