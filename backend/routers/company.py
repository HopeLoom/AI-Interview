...
from interview_configuration.models import CompanyData, CompanyDashboardData
...
async def get_company_dashboard(
    ...
    try:
        ...
        return {
            "success": True,
            "dashboard": dashboard_data.dict() if hasattr(dashboard_data, 'dict') else dashboard_data
        }
                ...
                for candidate in candidates:
                    ...
                    if sessions:
                        ...
                        if completed_sessions:
                            ...
                            # Get evaluation data
                            evaluation = await database.get_candidate_evaluation_data(candidate.user_id, recent_session.session_id)
...
async def get_company_job_postings(
    ...
    try:
        ...
        # Get job postings
        if status:
            job_postings = await db_service.search_job_postings(company_id=company_id, status=status)
        else:
...
def _extract_configuration_metadata(configuration: Dict[str, Any]) -> Dict[str, Any]:
    ...
    interview_data = simulation_config.get('interview_data') or simulation_config.get('interview') or {}
    ...
    return {
        ...
        "department": job_details.get('department') or interview_data.get('department') or "General",
    ...
    try:
        ...
        for configuration in configurations:
            ...
            last_activity_candidates = [
                session.get('updatedAt') or session.get('updated_at') or session.get('completedAt') or session.get('completed_at')
                for session in sessions
            ]
...
async def get_company_configuration_sessions(
    ...
    try:
        ...
        if not configuration or (configuration.get('companyId') != company_id and configuration.get('company_id') != company_id):
        ...
        try:
            ...
            if dashboard_data:
                ...
                dashboard = DashboardData(
                    ...
                    active_jobs=dashboard_data.get('active_jobs', 1),  # Default to 1 if not provided
                    total_evaluations=dashboard_data.get('total_evaluations', dashboard_data['completed_interviews'])  # Use completed interviews as fallback
                )
    ...
    try:
        ...
        try:
            ...
            if candidates is not None:
                ...
                raise HTTPException(status_code=404, detail="No candidates found for this interview")
