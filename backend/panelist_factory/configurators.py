...
async def create_panelist_instance(
    interview_config,
    llm_provider,
    gemini_provider,
    groq_provider,
    ...
    generator = PanelistGenerator(
        llm_provider, character_data, data_dir, user_id, firebase_database
    )
    config = await generator.generate_info()
    panelist_id = generate_id(user_id)
    config.id = panelist_id
    panelist = _configure_panelist(
        interview_config,
        config,
        user_id,
        firebase_user_id,
