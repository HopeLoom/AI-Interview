...
from activity_agent.base import BaseActivity, BaseActivityConfiguration, ActivityDetailsOutputMessage
...
from activity_agent.base import ActivityProgressAnalysisSummaryForPanelistOutputMessage, PromptInput
...
class Activity(BaseActivity):
    ...
    def __init__(self, 
        ...
        self.starter_code_data = self.load_activity_code_info()                     # Load answer code from file. we are not using this anywhere
        ...
        self.recent_progress:ActivityProgressAnalysisSummaryForPanelistOutputMessage = ActivityProgressAnalysisSummaryForPanelistOutputMessage()
        ...
        if (is_evaluating == False):
            ...
            for file_pattern in ["_progress_analysis_output.json", 
    ...
    async def load_activity_progress(self):
        activity_progress = await self.database.get_activity_progress_analysis_output_from_database(self.firebase_user_id, self.session_id)
        if activity_progress is None:
            ...
            self.recent_progress = ActivityProgressAnalysisSummaryForPanelistOutputMessage(**json.loads(activity_progress["activity_progress_summary_for_panelist"]))
    
    async def load_activity_code_from_candidate(self):
        self.activity_code_from_candidate = await self.database.get_recent_code_data(self.firebase_user_id)
        if self.activity_code_from_candidate is None:
            ...
            self.logger.info(f"Loaded activity code from candidate: {self.activity_code_from_candidate}")
    ...
    def parse_and_process_response_activity_progress(self, response: AssistantChatMessage, prompt: ChatMessage)->ActivityProgressAnalysisOutputMessage:
        try:
    ...
    def parse_and_process_response_activity_progress_with_respect_to_question(self, response: AssistantChatMessage, prompt: ChatMessage)->ActivityProgressWithRespectToQuestionOutputMessage:
        if response.content is None:
            return ActivityProgressWithRespectToQuestionOutputMessage()
        json_data = json.loads(response.content)
        self.logger.info(f"json_data inside activity progress with respect to question is: {json_data}")
        if "error" in json_data.keys():
            return ActivityProgressWithRespectToQuestionOutputMessage()
        output = ActivityProgressWithRespectToQuestionOutputMessage.model_validate(json_data)
        return output
    
    def parse_and_process_response_activity_progress_summary_for_panelist(self, response: AssistantChatMessage, prompt: ChatMessage)->ActivityProgressAnalysisSummaryForPanelistOutputMessage:
        if response.content is None:
    ...
    async def get_high_level_analysis(self, message:MasterMessageStructure) -> ActivityProgressAnalysisOutputMessage:
        
        prompt_input = PromptInput(response_type = ActivityPromptStrategy.RESPONSE_TYPE.ACTIVITY_HIGH_LEVEL_ANALYSIS,
                                   starter_code = self.starter_code_data if self.starter_code_data is not None else "",
                                   activity_code_from_candidate = message.activity_code_from_candidate if message.activity_code_from_candidate is not None else "",
                                   activity_progress_history = self.progress_history
                                   )

        prompt:ChatPrompt = super().build_prompt(prompt_input)
        output:ActivityProgressAnalysisOutputMessage =  await super().run_activity_progress_analysis(prompt)
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "activity_progress_analysis", output.model_dump() if hasattr(output, 'model_dump') else output)

        self.progress_history.append(output.code_intrepretation)
        return output
    
    async def get_activity_progress_analysis_with_respect_to_question(self, message:MasterMessageStructure, activity_progress_analysis) -> ActivityProgressWithRespectToQuestionOutputMessage:
        
        prompt_input = PromptInput( response_type = ActivityPromptStrategy.RESPONSE_TYPE.ACTIVITY_ANALYSIS_WITH_RESPECT_TO_QUESTION,
                            starter_code = self.starter_code_data if self.starter_code_data is not None else "",
        ...
        prompt:ChatPrompt = super().build_prompt(prompt_input)
        output:ActivityProgressWithRespectToQuestionOutputMessage =  await super().run_activity_progress_analysis_with_respect_question(prompt)
        self.logger.info(f"output from high level analysis with respect to question is: {output}")
        
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "activity_progress_with_respect_to_question", output.model_dump() if hasattr(output, 'model_dump') else output)
        
        with open(self.data_dir /  "_progress_analysis_with_respect_to_question_output.json", "w") as f:
            json.dump(output.model_dump(), f, indent=4)
            f.write("\n")

        return output
    
    async def get_activity_progress_analysis_summary_for_panelist(self, message:MasterMessageStructure, activity_progress_analysis, activity_progress_question) -> ActivityProgressAnalysisSummaryForPanelistOutputMessage:
        
        prompt_input = PromptInput( response_type = ActivityPromptStrategy.RESPONSE_TYPE.ACTIVITY_ANALYSIS_SUMMARY_FOR_PANELIST,
                                    starter_code = self.starter_code_data if self.starter_code_data is not None else "",
                                    activity_code_from_candidate = message.activity_code_from_candidate if message.activity_code_from_candidate is not None else "",
                                    activity_progress_history = self.progress_history,
                                    activity_progress_analysis = activity_progress_analysis,
        ...
        output:ActivityProgressAnalysisSummaryForPanelistOutputMessage =  await super().run_activity_progress_analysis_summary_for_panelist(prompt)
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "activity_progress_analysis_summary_for_panelist", output.model_dump() if hasattr(output, 'model_dump') else output)
    ...
    async def _save_activity_data(self):
        ...
        try:
            ...
            with open(self.data_dir / "_activity_code_from_candidate.json", "w") as f:
                json.dump({"activity_code_from_candidate": self.activity_code_from_candidate}, f, indent=4)
            
            # Save to database
            await self.database.add_json_data_output_to_database(
                self.firebase_user_id, self.session_id, "activity_code", {"code": self.activity_code_from_candidate}
            )
        except Exception as e:
    ...
    async def _run_analysis_pipeline(self, message: MasterMessageStructure):
        ...
        try:
            ...
            activity_progress_question = await self.get_activity_progress_analysis_with_respect_to_question(
    ...
    async def _set_no_progress_state(self):
        ...
        self.recent_progress.things_left_to_do_with_respect_to_question = "Candidate did not do anything"
        self.recent_progress.percentage_of_question_solved = 0.0
        
        try:
            await self.database.add_json_data_output_to_database(
                self.firebase_user_id, self.session_id, "activity_progress_analysis_summary_for_panelist_no_progress", self.recent_progress.model_dump() if hasattr(self.recent_progress, 'model_dump') else self.recent_progress
            )
        except Exception as e:
    ...
    async def run(self):
        ...
        while True:
            try:
                communicationMessage = self.receiving_message_queue.get_nowait()  # if there is no message, it should raise the queue empty exception
    ...
    def is_system_message(self, communicationMessage: CommunicationMessage) -> bool:
        ...
        return (communicationMessage.sender == SimulationRole.MASTER and 
                communicationMessage.receiver == SimulationRole.ALL)
