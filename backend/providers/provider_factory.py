from core.resource.model_providers.openai import OpenAIProvider, OpenAICredentials
from core.resource.model_providers.gemini import GeminiProvider, GeminiCredentials
from core.resource.model_providers.groq import GroqProvider, GroqCredentials 
from core.resource.model_providers.grok import GrokProvider, GrokCredentials
from core.resource.model_providers.deepseek import DeepSeekProvider, DeepSeekCredentials
from core.resource.model_providers.perplexity import PerplexityProvider, PerplexityCredentials
...
class ProviderFactory:
    ...
    def create_openai_provider(self):
        ...
        provider_config = next((p for p in config.llm_providers if p.name.lower() == 'openai'), None)
    ...
    def create_gemini_provider(self):
        ...
        provider_config = next((p for p in config.llm_providers if p.name.lower() == 'gemini'), None)
        if not provider_config or not provider_config.enabled:
    ...
    def create_deepseek_provider(self):
        ...
        provider_config = next((p for p in config.llm_providers if p.name.lower() == 'deepseek'), None)
        if not provider_config or not provider_config.enabled:
    ...
    def create_perplexity_provider(self):
        ...
        provider_config = next((p for p in config.llm_providers if p.name.lower() == 'perplexity'), None)
        if not provider_config or not provider_config.enabled:
