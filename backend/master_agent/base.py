...
from typing import List, DefaultDict, Any, Optional, Union
import enum
import abc
import typing
from abc import ABC, abstractmethod
from core.resource.model_providers.schema import ChatModelProvider, ChatModelInfo,ReasoningChatMessage, MasterChatMessage, SystemSettings
from core.resource.model_providers.openai import ChatPrompt
from core.resource.model_providers.openai import OpenAIModelName, OPENAI_CHAT_MODELS
from core.resource.model_providers.gemini import GeminiModelName, GEMINI_CHAT_MODELS
from core.resource.model_providers.groq import GroqModelName, GROQ_CHAT_MODELS
from core.resource.model_providers.grok import GrokModelName, GROK_CHAT_MODELS
from core.resource.model_providers.deepseek import DeepSeekModelNames, DEEPSEEK_CHAT_MODELS
from core.prompting.base import BaseMasterPromptStrategy
from interview_details_agent.base import InterviewRoundDetails, BaseInterviewConfiguration, ActivityDetailsOutputMessage, SubTopicData, InterviewTopicData, JobDetails
from activity_agent.base import ActivityProgressAnalysisSummaryForPanelistOutputMessage
from panelist_agent.base import Profile, Background
...
class TOPICS_TECHNICAL_ROUND(str, enum.Enum):
    ...
    PROBLEM_INTRODUCTION_AND_CLARIFICATION_AND_PROBLEM_SOLVING = "Problem Introduction, Clarification and Problem Solving Task"
...
class ConversationalAdviceInputMessage(BaseModel):
    ...
    topic_just_got_completed:bool = False
    section: str = ""
    speaker_determination_output:SpeakerDeterminationOutputMessage = SpeakerDeterminationOutputMessage()
    topic_completion_output:TopicSectionCompletionOutputMessage = TopicSectionCompletionOutputMessage()
...
class MasterMessageStructure(BaseModel):
    ...
    topic_completion_message:TopicSectionCompletionOutputMessage = TopicSectionCompletionOutputMessage()
    speaker_determination_message:SpeakerDeterminationOutputMessage = SpeakerDeterminationOutputMessage()
...
class QuestionSpecificEvaluationOutputMessage(BaseModel):
    question_criteria_specific_scoring:List[QuestionCriteriaSpecificScoring] = Field(default_factory=list)
...
class PromptInput(BaseModel):
    ...
    response_type:BaseMasterPromptStrategy.RESPONSE_TYPE = BaseMasterPromptStrategy.RESPONSE_TYPE.INTRO
    conversation_history_for_current_subtopic: List[MasterChatMessage] = Field(default_factory=list)
    last_completed_conversation_history: List[MasterChatMessage] = Field(default_factory=list)
    conversation_summary_for_current_topic: List[str] = Field(default_factory=list)
    conversation_summary_for_completed_topics: List[str] = Field(default_factory=list)
...
class BaseMaster(Configurable[BaseMasterConfiguration], ABC):
    ...
    async def run_speaker_determination_model(self, prompt:ChatPrompt) -> ChatModelResponse:
        response = await self.llm_provider.create_chat_completion(
            chat_messages = prompt,
            model_name = self.settings.slow_llm,
            completion_parser= lambda r: self.parse_and_process_response_speaker_determination(r, prompt),
            is_json_mode=True,
        )
        return response
    ...
    async def run_conversational_advice_model(self, prompt:ChatPrompt) -> ChatModelResponse:
        response = await self.llm_provider.create_chat_completion(
            chat_messages = prompt,
            model_name = self.settings.slow_llm,
            completion_parser= lambda r: self.parse_and_process_response_conversational_advice(r, prompt),
            is_json_mode=True
        )
        return response
    ...
    async def run_topic_summarizer(self, prompt:ChatPrompt) -> ChatModelResponse:
        response = await self.grok_provider.create_chat_completion(
            ...
            completion_parser= lambda r: self.parse_and_process_response_summarized_conversation(r, prompt),
            is_json_mode=True   
        )
        return response 
