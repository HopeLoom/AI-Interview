...
class PanelistGenerator():
    ...
    def __init__(self, llm_provider, character_data:CharacterData, data_dir, user_id, firebase_database):
    ...
    async def generate_info(self):
        ...
        profile_json_data = self.firebase_database.get_panelist_profile_json_data(self.character_data.character_name)
        ...
        panelist_config = BasePanelistConfiguration(id=profile.character_id, profile=profile, settings=settings, name=self.character_data.character_name, description=self.character_data.character_name)
