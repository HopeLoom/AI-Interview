        ...
        try:
            ...
            for user in users:
                try:
                    ...
                    session_id = await self.source_db.get_most_recent_session_id_by_user_id(user.user_id)
                    if not session_id:
                        continue
                    ...
                    if session_data:
                        ...
                        await self.target_db.update_session(user.user_id, session_id, {
                    ...
                    all_session_data = await self.source_db.get_all_session_data(user.user_id, session_id)
                    ...
                    if 'interview_transcript' in all_session_data:
                        for transcript_data in all_session_data['interview_transcript'].values():
                            ...
                            message = MockMessage(transcript_data['speaker'], transcript_data['dialog'])
                            await self.target_db.add_dialog_to_database(user.user_id, session_id, message)
        ...
        try:
            ...
            for config_info in configs:
                try:
                    ...
                    self.log_error(f"Failed to migrate configuration {config_info['config_name']}: {e}")
        ...
        try:
            ...
            for user in users:
                ...
                if user.simulation_config_json_url:
                    updates['simulation_config_json_url'] = self._convert_firebase_url(user.simulation_config_json_url)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--source", choices=["firebase", "postgresql", "sqlite"], required=True,
                       help="Source database type")
    parser.add_argument("--target", choices=["firebase", "postgresql", "sqlite"], required=True,
