    ...
    >
      <div className="flex items-center justify-between p-4 border-b border-slate-600 bg-slate-700">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-xs bg-blue-500/20 text-blue-300 px-2 py-1 rounded-full border border-blue-500/30">Coding Challenge</span>
        </div>
                ...
                {examples.map((example, index) => (
                  <div key={index} className="bg-slate-700 p-3 rounded-md border border-slate-600">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm text-slate-300"><span className="font-medium">Input:</span> <code className="bg-slate-600 px-1 py-0.5 rounded text-blue-300">{example.input}</code></p>
                      <p className="text-sm text-slate-300"><span className="font-medium">Output:</span> <code className="bg-slate-600 px-1 py-0.5 rounded text-green-300">{example.output}</code></p>
                      {example.explanation && (
                        <p className="text-sm text-slate-300"><span className="font-medium">Explanation:</span> {example.explanation}</p>
                      )}
                    </div>
                  </div>
                ))}
