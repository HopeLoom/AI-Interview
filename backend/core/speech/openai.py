    ...
    async def _text_to_speech(self, websocket_connection_manager, user_id, text, voice_name) -> Dict[str, Any]:
        ...
        try:
            async with httpx.AsyncClient(timeout=15.0) as client:
                async with client.stream("POST", self.config.tts_url, json=payload, headers=headers) as response:
                    ...
                    async for chunk in response.aiter_bytes():
                        if chunk:
                            try:
                                ...
                                if (websocket_connection_manager is not None and websocket_connection_manager.get_master_instance(user_id)):
