...
if config.storage.type == "local":
    ...
    print(f"Mounting static folder: {static_path} (from config: storage.type={config.storage.type})")
    app.mount("/static", StaticFiles(directory=static_path), name="static")
else:
    # For non-local storage (firebase, aws_s3), we still need a local static folder for temporary files
    static_path = os.path.join(BASE_DIR, "static")
    print(f"Using {config.storage.type} storage but mounting local static for temp files: {static_path}")
    app.mount("/static", StaticFiles(directory=static_path), name="static")
...
async def startup_event():
    ...
    websocket_handler = WebSocketHandler(websocket_connection_manager, user_master_instance_manager, data_dir, all_providers)
