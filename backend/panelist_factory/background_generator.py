...
class BackgroundGenerator():
    ...
    def build_user_prompt(self, character_data:CharacterData):
        user_prompt = ("The following is the information about the character for which background information has to be generated:\n"
    ...
    def parse_response_content(response):
        ...
        background.current_occupation = CurrentOccupation(**json.loads(json_data['current_occupation']))
        background.education = [Education(**json.loads(education)) for education in json_data['education']]
        background.experience = [Experience(**json.loads(experience)) for experience in json_data['experience']]
