...
from master_agent.base import BaseMaster, BaseMasterConfiguration
from core.resource.model_providers.schema import ChatMessage, AssistantChatMessage, ChatModelProvider, MasterChatMessage
from core.prompting.prompt_strategies.master_one_shot import MasterPromptStrategy
from master_agent.base import PanelData, CommunicationMessage, BaseInterviewConfiguration, SimulationRole, InterviewRound, PromptInput
from master_agent.base import SpeakerDeterminationOutputMessage, TopicSectionCompletionOutputMessage, TopicSectionCompletionInputMessage
from master_agent.base import ConversationalAdviceInputMessage, RulesAndRegulationsOutputMessage
from master_agent.base import SpeakerDeterminationOutputMessage, ConversationalAdviceOutputMessage, MasterMessageStructure, SlaveMessageStructure
from master_agent.base import ConversationalAdviceOutputMessage, SpeakerDeterminationInputMessage, SimulationIntroductionOutputMessage, SimulationIntroductionInputMessage, WebSocketMessageTypeToClient
from master_agent.base import ActivityDataToClient, InterviewStartDataToClient, InterviewEndDataToClient, InterviewMessageDataToClient, WebSocketMessageToClient, WebSocketMessageFromClient
from master_agent.base import InstructionDataToClient, InterviewDetails, NextSpeakerInfoToClient, SystemMessageStructure, SystemMessageType, WebSocketMessageTypeFromClient
from master_agent.interview_topic_tracker import InterviewTopicTracker
from interview_details_agent.base import InterviewTopicData, CharacterDataOutput, SubTopicData, ActivityDetailsOutputMessage
...
from core.resource.model_providers.schema import ChatModelResponse
...
from master_agent.base import TOPICS_HR_ROUND, TOPICS_TECHNICAL_ROUND, SUBTOPICS_TECHNICAL_ROUND
...
class Master(BaseMaster):
    config: BaseMasterConfiguration = BaseMasterConfiguration(description="Master configuration", name="Master")
    ...
    def __init__(self, 
        ...
        self.simulation_introduction_output_message:SimulationIntroductionOutputMessage = SimulationIntroductionOutputMessage()
        ...
        self.speech_service_provider = SpeechServiceProvider(config=SpeechConfig(provider="eleven_labs", speak_mode=True, api_key=self.config.speech.elevenlabs_api_key or ""), main_logger=self.logger)
        ...
        self.topic_completion_output_message:TopicSectionCompletionOutputMessage = TopicSectionCompletionOutputMessage()
        self.speaker_determination_output_message:SpeakerDeterminationOutputMessage = SpeakerDeterminationOutputMessage()
        self.conversational_advice_output_message:ConversationalAdviceOutputMessage = ConversationalAdviceOutputMessage()
        ...
        self.server_media_address = f"http://{self.address}:{self.port_number}/static/{self.user_id}"
    ...
    def cleanup_files(self):
        ...
        for file_pattern in ["_speaker_determination.json", "_topic_section_completion.json", "_rules_and_regulations.json", "_conversational_advice.json"]:
    ...
    def prepare_interview_details_data(self)->InterviewDetails:
        ...
        interview_details_data.expectations = ["Introduction", "Coding Challenge", "Technical Discussion"]
    ...
    def _get_voice_name(self, gender):
        ...
        if location.lower() == "india":
            male_options = self.speech_service_provider.get_available_voices(provider="eleven_labs", gender="male", region="india")
            female_options = self.speech_service_provider.get_available_voices(provider="eleven_labs", gender="female", region="india")
        else:
            male_options = self.speech_service_provider.get_available_voices(provider="eleven_labs", gender="male", region="us")
            female_options = self.speech_service_provider.get_available_voices(provider="eleven_labs", gender="female", region="us")
    ...
    def get_panelist_thoughts(self):
        ...
        for panelist in self.panelist_instances:
            if panelist.get_my_profile().interview_round_part_of == self.current_interview_round:
                panelist_thoughts_name_mapping[panelist.get_my_profile().background.name] = panelist.get_reasoning_history()
        
        return panelist_thoughts_name_mapping
    

    def set_panelist_data_for_frontend(self, simulation_introduction_output_message:SimulationIntroductionOutputMessage):
    ...
    async def _send_message_to_frontend(self, message:Any, message_type=None):
        ...
        if self.websocket_connection_manager is None:
            self.logger.warning("Websocket connection manager is not set. Cannot send message to frontend.")
            return
        ...
        await self.websocket_connection_manager.broadcast(websocket_message_to_client.model_dump_json())
    ...
    def _set_image_config(self):
        ...
        openai_provider = next((p for p in self.config.llm_providers if p.name.lower() == 'openai'), None)
    ...
    async def _create_candidate_agent(self):
        ...
        self.candidate_instance = await create_candidate_instance(llm_provider = self.llm_provider, 
                                                        resume_data = None, 
    ...
    async def _create_panelist_agents(self):
        ...
        for character_data in character_data_list:
            ...
            self.logger.info(f"Panelist {panelist_profile.background.name} created with voice id {voice_id}")
    ...
    async def _create_activity_agent(self):
        ...
        activity_configuration:BaseActivityConfiguration =  BaseActivityConfiguration(name="Activity", description="Activity", activity_code_file_path=self.config.interview_data.activity_code_path, activity_details=self.config.interview_data.activity_details)
        activity_configuration.activity_info_file_path = self.config.interview_data.activity_raw_data_path
    ...
    def send_command(self, command:CommunicationMessage):
        self.logger.info(f"Command sent to other agents") 
    ...
    def send_command_to_activity(self, command:CommunicationMessage):
        self.logger.info(f"Command sent to activity")
    ...
    def parse_and_process_response_introduction(self, response: AssistantChatMessage, prompt: ChatMessage):
        output:SimulationIntroductionOutputMessage = self.prompt_strategy.parse_response_introduction_content(response)
        return output
    
    def parse_and_process_response_summarized_conversation(self, response: AssistantChatMessage, prompt: ChatMessage):
        summary = self.prompt_strategy.parse_response_summarized_conversation_content(response)
        return summary

    def parse_and_process_response_conversational_advice(self, response: AssistantChatMessage, prompt: ChatMessage):
        advice_message:ConversationalAdviceOutputMessage = self.prompt_strategy.parse_response_advice_content(response)
        return advice_message
    
    def parse_and_process_response_rules_regulation(self, response: AssistantChatMessage, prompt: ChatMessage):
        rules_and_regulations_message:RulesAndRegulationsOutputMessage = self.prompt_strategy.parse_rules_and_regulation_content(response)
        return rules_and_regulations_message

    def parse_and_process_response_speaker_determination(self, response: AssistantChatMessage, prompt: ChatMessage):
        speaker_message:SpeakerDeterminationOutputMessage = self.prompt_strategy.parse_response_speaker_determination_content(response)
        return speaker_message
    
    def parse_and_process_response_topic_completion(self, response: AssistantChatMessage, prompt: ChatMessage):
        topic_completion:TopicSectionCompletionOutputMessage = self.prompt_strategy.parse_topic_section_completion_content(response)
    ...
    def get_conversation_data(self, topic_name, subtopic_name):
        ...
        conversation_history_for_current_subtopic = self.interview_topic_tracker.get_conversation_history_for_subtopic(
                                                            self.current_interview_round,
                                                            topic_name, 
                                                            subtopic_name)
        
        if conversation_history_for_current_subtopic is None:
            self.logger.info(f"Conversation history is None for topic: {topic_name} and subtopic: {subtopic_name}")
            conversation_history_for_current_subtopic = []
        
        last_completed_subtopic = self.interview_topic_tracker.get_last_completed_subtopic_name(self.current_interview_round, topic_name)
        
        if last_completed_subtopic is not None:
            last_completed_conversation_history = self.interview_topic_tracker.get_conversation_history_for_subtopic(
                                                            self.current_interview_round,
                                                            topic_name, 
                                                            last_completed_subtopic)
            if last_completed_conversation_history is None:
                self.logger.info(f"Last completed conversation history is None for topic: {topic_name} and subtopic: {last_completed_subtopic}")
                last_completed_conversation_history = []
        
        elif last_completed_subtopic is None:
            last_completed_topic = self.interview_topic_tracker.get_last_completed_topic_name(self.current_interview_round)
            if last_completed_topic is not None:
                last_completed_conversation_history = self.interview_topic_tracker.get_conversation_history_for_topic(self.current_interview_round, last_completed_topic)
            else:
                last_completed_conversation_history = []
        else:
            last_completed_conversation_history = []
        ...
        conversation_summary_for_completed_topics = self.interview_topic_tracker.get_topic_summary_of_all_completed_topics(self.current_interview_round)

        if conversation_summary_for_completed_topics is None:
            self.logger.info(f"Conversation summary is None for completed topics")
            conversation_summary_for_completed_topics = []

        return conversation_history_for_current_subtopic, last_completed_conversation_history, conversation_summary_for_current_topic, conversation_summary_for_completed_topics
    ...
    async def generate_subtopic_summary(self):
        ...
        conversation_history_for_current_subtopic, last_completed_conversation_history, conversation_summary_for_current_topic, conversation_summary_for_completed_topics = self.get_conversation_data(topic_name, subtopic_name)
        ...
        self.interview_topic_tracker.add_subtopic_summary_to_memory(self.current_interview_round, topic_name, subtopic_name, [summary])
        self.logger.info(f"Subtopic summary added to memory")
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, f"subtopic_summary_{topic_name}_{subtopic_name}", {
    ...
    async def generate_introduction(self)->SimulationIntroductionOutputMessage:
        ...
        simulation_introduction_message = SimulationIntroductionInputMessage(panelists=self.get_panelist_profiles_for_current_interview_round())
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "introduction_output", intro.model_dump() if hasattr(intro, 'model_dump') else intro)
        return intro
    ...
    async def speaker_determination(self)->SpeakerDeterminationOutputMessage:
        ...
        speaker_determination_message = SpeakerDeterminationInputMessage(panelists = panelist_profiles, 
                                                                        candidate_profile = self.get_candidate_profile(), 
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "speaker_determination_output", {
            "next_speaker": speaker_data.next_speaker,
            "reason": speaker_data.reason_for_selecting_next_speaker
        } if hasattr(speaker_data, 'next_speaker') else speaker_data.model_dump())
    ...
    async def check_section_topic_completion(self)->TopicSectionCompletionOutputMessage:        
        ...
        topic_completion_message = TopicSectionCompletionInputMessage(topic_data = self.current_topic_data, 
        ...
        remaining_subtopics = self.interview_topic_tracker.get_all_uncompleted_subtopics(self.current_interview_round, topic_name)
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "topic_section_completion_output", {
            "topic": topic_name,
            "decision": topic_section_completion.decision,
            "reason": topic_section_completion.reason
        } if hasattr(topic_section_completion, 'decision') else topic_section_completion.model_dump())
        ...
        if topic_section_completion.decision == 'YES': 
            ...
            self.interview_topic_tracker.update_topic_completion_status(self.current_interview_round, topic_name, subtopic_name, self.current_section_data)  # move to next topic
            self.topic_just_got_completed = True

            if (self.interview_topic_tracker.is_topic_completed(self.current_interview_round, topic_name)):
                self.logger.info("Topic completed")
            
            if (self.interview_topic_tracker.is_subtopic_completed(self.current_interview_round, topic_name, subtopic_name)):
                self.logger.info("Subtopic completed")
                await self.generate_subtopic_summary()  # generate conversation summary
                self.topic_start_time = datetime.now(timezone.utc)  # update the topic start time since a new topic has started
    ...
    async def get_conversational_advice(self, next_speaker:str)->ConversationalAdviceOutputMessage:
        self.logger.info("Generating conversational advice")

        speaker_profile = self._get_profile_from_name(next_speaker)
        conversational_advice_message = ConversationalAdviceInputMessage(next_speaker = speaker_profile if speaker_profile is not None else Profile(), 
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "conversational_advice_output", {
        ...
        } if hasattr(conversational_advice, 'advice_for_speaker') else conversational_advice.model_dump())
    ...
    def prepare_and_send_message(self,
                                ...
                                receiver = SimulationRole.PANELIST):

        speaker_profile = self._get_profile_from_name(speaker)
        topic_name = self.current_topic_data.name
        subtopic_name = self.current_subtopic_data.name

        remaining_subtopics_names = self.interview_topic_tracker.get_all_uncompleted_subtopics(self.current_interview_round, topic_name)
        ...
        message = MasterMessageStructure(
                ...
                evaluation_criteria = self.interview_topic_tracker.get_metrics_covered_for_current_interview_round(self.current_interview_round),
                activity_code_from_candidate = activity_code_from_candidate,
                current_section = self.current_section_data if self.current_section_data is not None else "",
        ...
        communication_message = CommunicationMessage.message_to_slave(sender = SimulationRole.MASTER, 
                                                                    receiver = receiver, 
                                                                    content = message)
    ...
    def prepare_and_send_system_message(self,
        ...
        # package this message into a communication message and send it to the speaker
        self.logger.info(f"Sending system message to all")
    ...
    async def handle_frontend_messages(self, message:WebSocketMessageFromClient):
        ...
        if message_type == WebSocketMessageTypeFromClient.INSTRUCTION:
            ...
            await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "metadata", {
            ...
            await self._send_message_to_frontend(instruction_data.model_dump_json(), WebSocketMessageTypeToClient.INSTRUCTION.value)
            return 
        ...
        # frontend if requesting for information before the interview starts. This will be done for the first time        
            ...
            self.prepare_and_send_system_message(SystemMessageType.START, "Interview round is started")
            ...
            interview_data.voice_name = self.tts_config.voice_id if self.tts_config.voice_id is not None else ""
            interview_data.participants = panelist_data
            if self.current_interview_round == InterviewRound.ROUND_ONE:
                interview_data.message = "Welcome to the prescreening round. We will now begin the HR interview round. This interview will be conducted by " + panelist_data[0].name +"." + panelist_data[0].intro
            else:
            ...
            await self._send_message_to_frontend(interview_data.model_dump_json(), WebSocketMessageTypeToClient.INTERVIEW_START.value)
        ...
        # frontend is requesting activity info so just send back the activity info. Activity info is the data corresponding to this question
            ...
            await self._send_message_to_frontend(data.model_dump_json(), WebSocketMessageTypeToClient.ACTIVITY_INFO.value)
            return
        ...
        # frontend is requesting for information when the interview round is ended
            ...
            topic, subtopic, section_name, is_interview_round_changed = self.interview_topic_tracker.get_topic_subtopic_for_discussion(self.current_interview_round)           
            if topic is None or subtopic is None:
                self.logger.error("Topic or subtopic is None")
                return
            
            if topic.name == TOPICS_TECHNICAL_ROUND.PROBLEM_INTRODUCTION_AND_CLARIFICATION_AND_PROBLEM_SOLVING.value:
                ...
                status = self.interview_topic_tracker.add_dialog_to_memory(self.current_interview_round, 
                                                                topic_name, 
                                                                subtopic_name,
                                                                masterchatmessage)
                if status:
                ...
                self.interview_topic_tracker.update_all_subtopics_within_topic_status(self.current_interview_round, topic_name)  # move to next topic
                if (self.interview_topic_tracker.is_topic_completed(self.current_interview_round, topic_name)):
                    self.logger.info("*** Topic completed ***")
                    self.topic_just_got_completed = True
        ...
        # candidate input is received from the frontend. this can be in response or if they press ask
            ...
            if (len(user_response) > 0):
                topic, subtopic, _, is_interview_round_changed = self.interview_topic_tracker.get_topic_subtopic_for_discussion(self.current_interview_round)           
                ...
                await self.database.add_dialog_to_database(self.firebase_user_id, self.session_id, masterchatmessage)
    ...
    async def process_and_send_message(self):
        ...
        if len(self.interview_topic_tracker.get_conversation_history_for_topic(self.current_interview_round, topic_name)) == 0:
        ...
        results = await asyncio.gather(
            ...
            *[self.get_conversational_advice(panelist_profile.background.name) for panelist_profile in panelist_profiles]  # run advice generation in parallel
        )
        ...
        if speaker:
            ...
            if speaker.lower() == self.get_candidate_profile().background.name.lower():
                ...
                await self._send_message_to_frontend(interview_message_data_to_be_sent.model_dump_json(), WebSocketMessageTypeToClient.INTERVIEW_DATA.value)
                self.waiting_for_response_from_candidate = True
                self.logger.info("Waiting for response from candidate")
            else:
                await self._send_message_to_frontend(next_speaker_info_message_to_be_sent.model_dump_json(), WebSocketMessageTypeToClient.NEXT_SPEAKER_INFO.value)
                possible_speakers = [panelist_profile.background.name for panelist_profile in panelist_profiles]
    ...
    async def process_frontend_messages(self):
        while True:
            try:
                websocketmessagefromclient = self.messages_from_frontend.get_nowait()      # pop out the message from the queue
                async with self.lock:
                    self.logger.info(f"Received message from frontend: {websocketmessagefromclient}")
                    await self.handle_frontend_messages(websocketmessagefromclient)    # here we can either respond back, update the user state or do some other processing
    ...
    def is_ready_for_next_message(self) -> bool:
        return (not self.waiting_for_response_from_panelist and 
                not self.waiting_for_response_from_candidate and 
    ...
    async def run(self):
        try:
            ...
            await self._send_message_to_frontend(interview_details_data.model_dump_json(), WebSocketMessageTypeToClient.INTERVIEW_DETAILS.value)
            ...
            while True:
                async with self.lock:
                    ...
                    if self.candidate_state_tracker == WebSocketMessageTypeFromClient.INTERVIEW_START:
                        ...
                        if (self.is_ready_for_next_message()):
                            ...
                            topic, subtopic, section, interview_round_changed = self.interview_topic_tracker.get_topic_subtopic_for_discussion(self.current_interview_round)  
                            if topic is None or subtopic is None:
                                self.logger.error("Topic or subtopic is None")
                                return
                            self.logger.info(f"Topic: {topic}, Subtopic: {subtopic}, Section: {section}")
                            self.logger.info(f"Current interview round: {self.current_interview_round}")
                            ...
                            if self.is_interview_round_changed:
                                ...
                                self.prepare_and_send_system_message(SystemMessageType.INTERVIEW_ROUND_CHANGED, "Interview round is changed from 1 to 2")
                                ...
                                interview_data.message += "This interview will be conducted by " + interview_data.participants[0].name + "." + interview_data.participants[0].intro 
                                interview_data.message += "The other interviewer will be " + interview_data.participants[1].name + "." + interview_data.participants[1].intro
                                await self._send_message_to_frontend(interview_data.model_dump_json(), WebSocketMessageTypeToClient.INTERVIEW_START.value)
                                self.is_audio_playback_completed = False
                                continue
                ...
                "is_interview_completed": self.is_interview_completed
                        ...
                        # we are waiting for response from the panelist.
                        elif (self.waiting_for_response_from_panelist == True) and self.is_audio_playback_completed == True:
                            ...
                            if self.receiving_message_queue.empty() == False:
                                ...
                                try:
                                    response:CommunicationMessage = self.receiving_message_queue.get_nowait()           # this will be of type CommunicationMessage
                                except asyncio.QueueEmpty:
                                ...
                                if sender == SimulationRole.PANELIST and receiver == SimulationRole.MASTER:
                                    
                                    self.waiting_for_response_from_panelist = False                             # we have received the response
                                    self.is_audio_playback_completed = False                                    # we will start the audio playback now
                                    
                                    if isinstance(response.content, SlaveMessageStructure):
                                        response_data:SlaveMessageStructure = response.content
                                    else:
                                        self.logger.error("Response is not of type SlaveMessageStructure")
                                        return
                                    ...
                                    status = self.interview_topic_tracker.add_dialog_to_memory(self.current_interview_round, self.current_topic_data.name, self.current_subtopic_data.name, chatMessage)
                                    if status:
                                        self.logger.info("Dialog added to memory")
                                    else:
                                        self.logger.error("Dialog not added to memory")
                                    await self.database.add_dialog_to_database(self.firebase_user_id, self.session_id, chatMessage)
                                    ...
                                    message_to_be_sent.interview_round = self.current_interview_round
                                    ...
                                    message_to_be_sent.current_subtopic = self.current_subtopic_data.name

                                    await self._send_message_to_frontend(message_to_be_sent.model_dump_json(), WebSocketMessageTypeToClient.INTERVIEW_DATA.value)
                                    
                    elif self.candidate_state_tracker == WebSocketMessageTypeFromClient.INTERVIEW_END:
                        ...
                        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "memory_graph.json", memory_graph.model_dump())
                        
                        data:InterviewEndDataToClient = InterviewEndDataToClient()
                        data.message = "Thank you for participating. Your Interview is completed !!!"
                        data.voice_name = self.tts_config.voice_id if self.tts_config.voice_id is not None else ""
                        await self._send_message_to_frontend(data.model_dump_json(), WebSocketMessageTypeToClient.INTERVIEW_END.value)
                        self.candidate_state_tracker = WebSocketMessageTypeFromClient.USER_LOGIN        # setting to something random

                    elif self.candidate_state_tracker == WebSocketMessageTypeFromClient.USER_LOGOUT:
                        self.prepare_and_send_system_message(SystemMessageType.END, "Interview is completed")
                        self.reset()
                        break
            ...
            await self._send_message_to_frontend("There was an error.", WebSocketMessageTypeToClient.ERROR.value)
