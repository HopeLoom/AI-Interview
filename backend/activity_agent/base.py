...
class ActivityProgressWithRespectToQuestionOutputMessage(BaseModel):
    code_intrepretation_with_respect_to_question: str = Field(
        default="", description="Code interpretation with respect to question"
    )
    logic_analysis_with_respect_to_question: str = Field(
        default="", description="Logic analysis with respect to question"
    )
    complexity_analysis_with_respect_to_question: str = Field(
        default="", description="Complexity analysis with respect to question"
    )
    remaining_things_to_do_with_respect_to_question: str = Field(
        default="", description="Remaining tasks with respect to question"
    )


class ActivityProgressAnalysisSummaryForPanelistOutputMessage(BaseModel):
    candidate_performance_summary: str = Field(
        default="", description="Summary of candidate performance"
    )
    percentage_of_question_solved: float = Field(
        default=DEFAULT_PERCENTAGE, ge=0, le=100, description="Percentage of question solved"
    )
    things_left_to_do_with_respect_to_question: str = Field(
        default="", description="Remaining tasks for the question"
    )


class BaseActivityConfiguration(SystemSettings):
    activity_name: str = Field(default="", description="Activity name")
    settings: ActivitySettings = Field(
        default_factory=ActivitySettings, description="Activity settings"
    )
    activity_details: ActivityDetailsOutputMessage = Field(
        default_factory=ActivityDetailsOutputMessage, description="Activity details"
    )
...
class BaseActivity(Configurable[BaseActivityConfiguration], ABC):
    ...
    async def run_activity_progress_analysis(
    ...
    async def run_activity_progress_analysis_with_respect_question(
        self, prompt: ChatPrompt
    ...
    async def run_activity_progress_analysis_summary_for_panelist(
        self, prompt: ChatPrompt
    ...
    def get_config_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            if hasattr(self.config, "activity_name")
            else "Unknown",
