...
from master_agent.base import SimulationIntroductionOutputMessage, MasterChatMessage, EvaluationMessageToFrontEnd, QuestionSpecificEvaluationOutputMessage, SpeakerDeterminationOutputMessage, ConversationalAdviceOutputMessage, TopicSectionCompletionOutputMessage
from master_agent.base import CandidateEvaluationVisualisationReport
from evaluation_agent.base import SubqueryDataExtractionOutputMessage, SubqueryGeneratorOutputMessage
from panelist_agent.base import ReasoningOutputMessage, DomainKnowledgeOutputMessage, ResponseOutputMessage, EvaluationOutputMessage, ResponseWithReasoningOutputMessage
from activity_agent.base import ActivityProgressAnalysisOutputMessage, ActivityProgressAnalysisSummaryForPanelistOutputMessage, ActivityProgressWithRespectToQuestionOutputMessage
...
class FireBaseDataBase:
    def __init__(self, logger=None, credentials_path: Optional[str] = None, storage_bucket: Optional[str] = None):
        self.user_data:UserProfile = UserProfile(user_id="", name="", email="", company_name="", job_title="", location="", auth_code="")
        self.logger = logger
    ...
    def _initialize_firebase(self, credentials_path: Optional[str] = None, storage_bucket: Optional[str] = None):
        ...
        try:
            ...
            if not storage_bucket:
                storage_bucket = os.getenv('FIREBASE_STORAGE_BUCKET', 'interview-simulation-c96c7.firebasestorage.app')
    ...
    def commit_batch(self):
        ...
        try:
            ...
            for operation in self.pending_batch_operations:
                doc_ref = self.db.collection("users").document(operation["user_id"]).collection("sessions").document(operation["session_id"]).collection(operation["collection_path"]).document()
    ...
    def create_new_session(self, user_id):
        ...
        session_id = datetime.now().strftime("%Y%m%d-%H%M%S")  # Generate a session ID (timestamp-based)
    ...
    def add_activity_progress_analysis_output_to_database(self, user_id, session_id, output:ActivityProgressAnalysisOutputMessage):
        """Add activity progress analysis output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("activity_progress_analysis_output")
    ...
    def add_activity_progress_analysis_summary_for_panelist_output_to_database(self, user_id, session_id, output:ActivityProgressAnalysisSummaryForPanelistOutputMessage):
        """Add activity progress analysis summary for panelist output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("activity_progress_analysis_summary_for_panelist_output")
        ...
        if self.logger is not None:
            self.logger.info("Activity progress analysis summary for panelist output added successfully.")

    def get_activity_progress_analysis_output_from_database(self, user_id, session_id) -> Optional[Dict[str, Any]]:
    ...
    def add_activity_code_to_database(self, user_id, session_id, activity_code):
        ...
        activity_code_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("activity_code")
    ...
    def add_activity_progress_with_respect_to_question_output_to_database(self, user_id, session_id, output:ActivityProgressWithRespectToQuestionOutputMessage):
        """Add activity progress with respect to question output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("activity_progress_with_respect_to_question_output")
        ...
        if self.logger is not None:
            self.logger.info("Activity progress with respect to question output added successfully.")   
    

    def add_subtopic_summary_to_database(self, user_id, session_id, current_interview_round, topic_name, subtopic_name, summary):
        """Add subtopic summary to Firestore"""
        summary_id = datetime.now().isoformat()

        summary_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("subtopic_summary")
    ...
    def add_panelist_evaluation_to_database(self, user_id, session_id, panelist_name, evaluation_output:EvaluationOutputMessage):
        """Add panelist evaluation output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("panelist_evaluation")

        output_data = {
            "panelist": panelist_name,
            "evaluation": evaluation_output.model_dump_json()
        }
    ...
    def add_topic_summary_to_database(self, user_id, session_id, current_interview_round, topic_name, summary_data):
        """Add topic summary to Firestore"""
        summary_id = datetime.now().isoformat()

        summary_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("topic_summary")
    ...
    def add_panelist_reasoning_to_database(self, user_id, session_id, panelist_name, reasoning_output:ReasoningOutputMessage):
        """Add panelist reasoning output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("panelist_reasoning")

        output_data = {
            "panelist": panelist_name,
            "reasoning": reasoning_output.model_dump_json()
        }
    ...
    def add_panelist_domain_knowledge_to_database(self, user_id, session_id, panelist_name, domain_knowledge_output:DomainKnowledgeOutputMessage):
        """Add panelist domain knowledge output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("panelist_domain_knowledge")
    ...
    def add_panelist_response_to_database(self, user_id, session_id, panelist_name, response_output:ResponseOutputMessage):
        """Add panelist response output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("panelist_response")

        output_data = {
            "panelist": panelist_name,
            "response": response_output.model_dump_json()
        }
    ...
    def add_panelist_response_with_reasoning_to_database(self, user_id, session_id, panelist_name, output:ResponseWithReasoningOutputMessage):
        """Add panelist response with reasoning output to Firestore"""
        output_id = datetime.now().isoformat()
        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("panelist_response_with_reasoning")
        output_data = {
            "panelist": panelist_name,
    ...
    def add_introduction_output_to_database(self, user_id, session_id, output:SimulationIntroductionOutputMessage):
        """Add simulation introduction output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("simulation_introduction_output")

        panelist_json_data = [panelist.model_dump_json() for panelist in output.panelists]
        output_data = {
            "introduction": output.introduction,
            "panelists": panelist_json_data
        }
    ...
    def add_dialog_to_database(self, user_id, session_id, message:MasterChatMessage):
        ...
        conversation_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("interview_transcript")
        ...
        if self.logger is not None:
            self.logger.info(f"Message added to conversation: {message.speaker} - {message.content}")

    def add_speaker_determination_output_to_database(self,user_id, session_id, output:SpeakerDeterminationOutputMessage):
        """Add speaker determination output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("speaker_determination_output")
    ...
    def add_conversational_advice_output_to_database(self,user_id, session_id, output:ConversationalAdviceOutputMessage):
        """Add conversational advice output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("conversational_advice_output")
    ...
    def add_topic_section_completion_output_to_database(self, user_id, session_id, topic_name, output:TopicSectionCompletionOutputMessage):
        
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("topic_section_completion_output")
    ...
    def add_evaluation_output_to_database(self, user_id, session_id, output:QuestionSpecificEvaluationOutputMessage):
        """Add evaluation output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("evaluation_output")
        
        question_criteria_specific_scoring_json_list = [criteria_scoring.model_dump_json() for criteria_scoring in output.question_criteria_specific_scoring] if output.question_criteria_specific_scoring is not None else []
    ...
    def add_subquery_data_to_database(self, user_id, session_id, output:SubqueryGeneratorOutputMessage):
        """Add subquery data to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("subquery_data")

        output_data = {
            "subquery_data": output.model_dump_json()
        }
        output_ref.document(output_id).set(output_data)
        if self.logger is not None:
    ...
    def add_subquery_data_extraction_output_to_database(self, user_id, session_id, output:SubqueryDataExtractionOutputMessage):
        """Add subquery data extraction output to Firestore"""
        output_id = datetime.now().isoformat()

        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("subquery_data_extraction_output")

        output_data = {
            "subquery_data_extraction": output.model_dump_json()
        }
    ...
    def add_metadata_to_database(self, user_id, session_id, is_interview_completed):
        ...
        metadata_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("metadata")

        metadata_data = {
            "is_interview_completed": is_interview_completed
        }
    ...
    def get_metadata_from_database(self, user_id, session_id):
        ...
        metadata_ref = self.db.collection("users").document(user_id) \
    ...
    def add_final_evaluation_output_to_database(self, user_id, session_id, output:EvaluationMessageToFrontEnd):
        """Add final evaluation output to Firestore"""
        output_id = datetime.now().isoformat()
        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("final_evaluation_output")
        output_data = {
            "final_evaluation": output.model_dump_json()
        }
        output_ref.document(output_id).set(output_data)
        if self.logger is not None:
    ...
    def add_final_visualisation_report_to_database(self, user_id, session_id, output:CandidateEvaluationVisualisationReport):
        """Add final visualisation report to Firestore"""
        output_id = datetime.now().isoformat()
        output_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("final_visualisation_report")
        output_data = {
            "visualisation_report": output.model_dump_json()
        }
        output_ref.document(output_id).set(output_data)
        if self.logger is not None:
    ...
    def add_json_data_output_to_database(self, user_id, session_id, name, json_data):
        ...
        output_id = datetime.now().isoformat()
        json_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection(name)
    ...
    def fetch_starter_code_from_url(self) -> Optional[str]:
        ...
        response = requests.get(updated_url)
        if response.status_code == 200:
            return response.content.decode("utf-8") if isinstance(response.content, bytes) else response.content
        else:
            if self.logger is not None:
                self.logger.error(f"Failed to fetch starter code. Status Code: {response.status_code}")
            return None
    ...
    def upload_image(self, image_path, user_id, file_name):
        ...
        blob = self.bucket.blob(f"users/{user_id}/{file_name}")  # Example: users/{user_id}/profile_pic.jpg
        blob.upload_from_filename(image_path, content_type="image/jpeg")  # Change content_type based on format
    ...
    def upload_file(self, file_path, user_id, file_name):
        ...
        blob = self.bucket.blob(f"users/{user_id}/{file_name}")  # Folder structure: users/{user_id}/
    ...
    def get_all_session_data(self, user_id, session_id = None) -> dict:
        ...
        try:
            session_doc_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id)
    ...
    def get_recent_code_data(self, user_id):
        ...
        try:
            ...
            code_ref = self.db.collection("users").document(user_id).collection("sessions").document(session_id).collection("activity_code")
    ...
    def create_company(self, company_data):
        ...
        try:
            ...
            if self.logger is not None:
                self.logger.error(f"Error creating company {company_data.get('company_id', 'unknown')}: {e}")
            return False
    ...
    def search_companies_by_name(self, name):
        ...
        try:
            ...
            query = companies_ref.where("name", ">=", name.lower()).where("name", "<=", name.lower() + "\uf8ff")
            results = query.stream()
    ...
    def get_candidate_evaluation_data(self, user_id, session_id=None):
        ...
        try:
            if session_id:
                ...
                evaluation_data = self.get_final_evaluation_output_from_database(user_id, session_id)
                ...
                if recent_session_id:
                    evaluation_data = self.get_final_evaluation_output_from_database(user_id, recent_session_id)
                    return evaluation_data
            return None
        except Exception as e:
            if self.logger is not None:
        ...
        try:
            ...
            for candidate in candidates:
                ...
                if sessions:
                    ...
                    has_completed = any(session.get('status') == 'completed' for session in sessions)
                    if has_completed:
                        ...
                        for session in sessions:
                            if session.get('status') == 'completed':
                                evaluation = self.get_candidate_evaluation_data(user_id, session['session_id'])
                                if evaluation and 'final_evaluation' in evaluation:
                                    try:
                                        ...
                                        if self.logger is not None:
                                            self.logger.warning(f"Error parsing evaluation data: {e}")
                                break
                    else:
            ...
            average_score = round(total_score / scored_interviews, 2) if scored_interviews > 0 else 0
            ...
            if self.logger is not None:
                self.logger.info(f"Generated dashboard data for company {company_id}: {total_candidates} candidates, {completed_interviews} completed")
    ...
    def get_candidates_with_evaluations(self, company_id):
        ...
        try:
            ...
            for candidate in candidates:
                ...
                if sessions:
                    ...
                    if completed_sessions:
                        ...
                        # Get evaluation data
                        evaluation = self.get_candidate_evaluation_data(user_id, recent_session.get('session_id'))
                        if evaluation and 'final_evaluation' in evaluation:
                            try:
                                eval_data = json.loads(evaluation['final_evaluation'])
            ...
            if self.logger is not None:
                self.logger.info(f"Retrieved {len(candidates_with_evaluations)} candidates with evaluations for company {company_id}")
            
            return candidates_with_evaluations
        except Exception as e:
            if self.logger is not None:
                self.logger.error(f"Error getting candidates with evaluations for company {company_id}: {e}")
            return []
        ...
        try:
                ...
                if user_id:
                    ...
                    if completed_sessions:
                        ...
                        for session in completed_sessions:
                            evaluation = self.get_candidate_evaluation_data(user_id, session.get('session_id'))
                            if evaluation and 'final_evaluation' in evaluation:
                                try:
                                    import json
                                    eval_data = json.loads(evaluation['final_evaluation'])
                                    if 'overall_score' in eval_data:
                                        job_interviews[job_title]['scores'].append(eval_data['overall_score'])
                                except (json.JSONDecodeError, KeyError):
                                    pass
                                break
            ...
            for job_title, data in job_interviews.items():
                ...
                interview = {
                    ...
                    'status': 'active',
                    'created_date': min([c.get('created_at', '') for c in data['candidates']] + [datetime.now().isoformat()]),
                    'last_activity': max([c.get('created_at', '') for c in data['candidates']] + [datetime.now().isoformat()]),
    ...
    def _get_department_from_title(self, job_title):
        ...
        if any(tech in title_lower for tech in ['engineer', 'developer', 'software', 'frontend', 'backend', 'fullstack']):
            return 'Engineering'
