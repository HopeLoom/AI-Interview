...
class ChromaStorageConnector(StorageConnector):
    ...
    def query(self, query:str, query_vec, top_k:int, filters:dict):
        ...
        results = self.collection.query(query_embeddings=[query_vec], n_results=top_k, include=self.include, where=chroma_filters)
        return results
    
    def query_text(self, query:str, top_k:int, filters:dict):
        ids, chroma_filters = self.get_filters()
        results = self.collection.query(query_texts=[query], n_results=top_k, include=self.include, where=chroma_filters)
        return results
