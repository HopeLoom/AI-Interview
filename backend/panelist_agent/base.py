...
from core.resource.model_providers.schema import ChatModelProvider, ChatModelInfo, SystemSettings, ReflectionChatMessage, MasterChatMessage, ChatModelResponse
...
class Profile(BaseModel):
    ...
    interview_round_part_of: str = Field(default="", description="Interview round this panelist belongs to")
...
class BasePanelistConfiguration(SystemSettings):
    ...
    settings: PanelistSettings = Field(default_factory=PanelistSettings, description="Panelist settings")
...
class ResponseWithReasoningOutputMessage(BaseModel):
    ...
    should_i_ask_a_new_question: bool = Field(default=False, description="Whether to ask new question")
    are_my_questions_too_repetitive: bool = Field(default=False, description="Repetitive questions flag")
    areas_to_cover_in_next_response: List[str] = Field(default_factory=list, description="Areas to cover")
    facts_corresponding_to_areas_to_cover_in_next_response: List[str] = Field(default_factory=list, description="Facts for areas")
    areas_already_covered: List[str] = Field(default_factory=list, description="Already covered areas")
    response: str = Field(default="", description="Generated response")
...
class DomainKnowledgeOutputMessage(BaseModel):
    ...
    relevance_to_conversation: str = Field(default="", description="Relevance to current conversation")
...
class ReasoningOutputMessage(BaseModel):
    ...
    facts_corresponding_to_areas_to_cover_in_next_response: List[str] = Field(default_factory=list, description="Facts for areas")
    areas_already_covered: List[str] = Field(default_factory=list, description="Already covered areas")
    is_domain_knowledge_access_needed: bool = Field(default=False, description="Domain knowledge needed")
...
class CriteriaSpecificScoring(BaseModel):
    ...
    key_phrases_from_conversation: List[str] = Field(default_factory=list, description="Key phrases from conversation")

class EvaluationOutputMessage(BaseModel):
    feedback_to_the_hiring_manager_about_candidate: str = Field(default="", description="Feedback for hiring manager")
    score: float = Field(default=DEFAULT_SCORE, ge=0, le=100, description="Overall evaluation score")

class PromptInput(BaseModel):
    response_type: BasePanelistPromptStrategy.RESPONSE_TYPE = Field(
        default=BasePanelistPromptStrategy.RESPONSE_TYPE.REASON, 
    ...
    reason: ReasoningOutputMessage = Field(default_factory=ReasoningOutputMessage, description="Reasoning output")
    domain_knowledge: DomainKnowledgeOutputMessage = Field(default_factory=DomainKnowledgeOutputMessage, description="Domain knowledge")
    reflection_history: List[ReflectionChatMessage] = Field(default_factory=list, description="Reflection history")
...
class BasePanelist(Configurable[BasePanelistConfiguration], ABC):
    ...
    async def respond_with_reasoning(self, prompt:ChatPrompt) -> ResponseWithReasoningOutputMessage:
    ...
    @abstractmethod
    def parse_process_reason_model(self, response: ChatModelResponse, prompt: ChatPrompt) -> ReasoningOutputMessage:
        """Parse reasoning model response"""
        pass

    @abstractmethod
    def parse_process_evaluate_model(self, response: ChatModelResponse, prompt: ChatPrompt) -> EvaluationOutputMessage:
        """Parse evaluation model response"""
        pass

    @abstractmethod
    def parse_process_response_model(self, response: ChatModelResponse, prompt: ChatPrompt) -> ResponseOutputMessage:
        """Parse response model response"""
        pass

    @abstractmethod
    def parse_process_reflect_model(self, response: ChatModelResponse, prompt: ChatPrompt) -> ReflectionOutputMessage:
        """Parse reflection model response"""
        pass

    @abstractmethod
    def parse_process_domain_knowledge_model(self, response: ChatModelResponse, prompt: ChatPrompt) -> DomainKnowledgeOutputMessage:
        """Parse domain knowledge model response"""
        pass

    @abstractmethod
    def parse_process_respond_with_reasoning_model(self, response: ChatModelResponse, prompt: ChatPrompt) -> ResponseWithReasoningOutputMessage:
