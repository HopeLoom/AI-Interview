...
class TopicSubTopicGraph(BaseModel):
    ...
    def get_all_subtopics_conversation_memory(self, topic_name):
        ...
        for subtopic_name in self.topic_subtopic_graph[topic_name].subtopics:
            conversation_history = self.topic_subtopic_graph[
                topic_name
            ].get_subtopic_conversation_memory(subtopic_name)
...
class MemoryGraph(BaseModel):
    ...
    def get_topic_node_from_name(
        self, interview_round: str, topic_name: str
    ) -> Optional[TopicSubTopicGraph]:
    ...
    def add_dialog_to_memory(
        self, interview_round: str, topic_name: str, subtopic_name: str, item: MasterChatMessage
    ) -> bool:
        ...
        try:
            ...
            if topic_node and topic_name in topic_node.topic_subtopic_graph:
                topic_node.topic_subtopic_graph[topic_name].subtopics[subtopic_name].add_to_memory(
                    item
                )
                return True
            return False
    ...
    def add_multiple_dialogs_to_memory(
        self,
        interview_round: str,
        topic_name: str,
        subtopic_name: str,
        ...
        topic_node = self.get_topic_node_from_name(interview_round, topic_name)
        if topic_node and topic_name in topic_node.topic_subtopic_graph:
            subtopic_memory = topic_node.topic_subtopic_graph[topic_name].subtopics.get(
                subtopic_name
            )
            if subtopic_memory:
    ...
    def get_topic_statistics(self, interview_round: str, topic_name: str) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "total_messages": sum(
                len(sub.conversation_memory) for sub in topic_memory.subtopics.values()
            ),
            "subtopics_with_summaries": sum(
                1 for sub in topic_memory.subtopics.values() if sub.summary
            ),
        }
        return stats
