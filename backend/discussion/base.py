...
from resource.model_providers.schema import ChatModelProvider, ChatModelInfo, SystemSettings, ChatMessage
from resource.model_providers.openai import OpenAIModelName, OPENAI_CHAT_MODELS
from core.prompting.base import DiscussionPromptStrategy
from activity.base import ActivityCodeGenerationOutputMessage, ActivityDetailsOutputMessage, BaseActivityConfiguration
...
class BaseDiscussion(Configurable[BaseDiscussionConfiguration], ABC):
    ...
    async def run_model(self, prompt, response_type):
        if response_type == DiscussionPromptStrategy.RESPONSE_TYPE.FEEDBACK_INFO:
            response = await self.llm_provider.create_chat_completion(
                ...
                completion_parser = lambda r: self.parse_and_process_response_feedback_info(r, prompt),
                is_json_mode = True
            )
            ...
            response = await self.llm_provider.create_chat_completion(
                chat_messages = prompt,
                model_name = self.get_llm_info().name,
                completion_parser = lambda r: self.parse_and_process_response_activity_info(r, prompt),
                is_json_mode = True
            )
            ...
            response = await self.llm_provider.create_chat_completion(
                chat_messages = prompt,
                model_name = self.get_llm_info().name,
                completion_parser = lambda r: self.parse_and_process_response_activity_code_info(r, prompt),
                is_json_mode = True
            )
            ...
            response = await self.llm_provider.create_chat_completion(
                chat_messages = prompt,
                model_name = self.get_llm_info().name,
                completion_parser = lambda r: self.parse_and_process_response_consensus_info(r, prompt),
                is_json_mode = True
            )
    ...
    async def get_feedback_information(self, prompt:ChatMessage) -> FeedbackOutput:
        response = await self.run_model(prompt, DiscussionPromptStrategy.RESPONSE_TYPE.FEEDBACK_INFO)
        return response
    
    async def get_activity_details(self, prompt:ChatMessage) -> ActivityDetailsOutputMessage:
        response = await self.run_model(prompt, DiscussionPromptStrategy.RESPONSE_TYPE.ACTIVITY_INFO)
        return response
    
    async def get_activity_code_details(self, prompt:ChatMessage) -> ActivityCodeGenerationOutputMessage:
        response = await self.run_model(prompt, DiscussionPromptStrategy.RESPONSE_TYPE.ACTIVITY_CODE_INFO)
        return response
    
    async def get_consensus_details(self, prompt:ChatMessage) -> ConsensusOutput:
        response = await self.run_model(prompt, DiscussionPromptStrategy.RESPONSE_TYPE.CONSENSUS_INFO)
        return response
