...
class PromptInput(BaseModel):
    ...
    response_type: BaseInterviewPromptStrategy.RESPONSE_TYPE = (
        BaseInterviewPromptStrategy.RESPONSE_TYPE.CHARACTER_INFO
    )
...
class BaseInterview(Configurable[BaseInterviewConfiguration], ABC):
    ...
    async def run_model(self, prompt, response_type):
        if response_type == BaseInterviewPromptStrategy.RESPONSE_TYPE.CHARACTER_INFO.value:
            response = await self.llm_provider.create_chat_completion(
                ...
                model_name="gpt-4o",
                completion_parser=lambda r: self.parse_and_process_response_character_info(
                    r, prompt
                ),
                is_json_mode=True,
            )
            ...
            response = await self.llm_provider.create_chat_completion(
                chat_messages=prompt,
                model_name="gpt-4o",
                completion_parser=lambda r: self.parse_and_process_response_activity_details_scenario(
                    r, prompt
                ),
                is_json_mode=True,
            )
    ...
    async def get_character_information(self, prompt: ChatMessage) -> CharacterDataOutput:
        response = await self.run_model(
            prompt, BaseInterviewPromptStrategy.RESPONSE_TYPE.CHARACTER_INFO
        )
        return response

    async def get_activity_details(self, prompt: ChatMessage) -> ActivityDetailsOutputMessage:
        response = await self.run_model(
            prompt, BaseInterviewPromptStrategy.RESPONSE_TYPE.ACTIVITY_DETAILS
        )
        return response

    async def get_starter_code(self, prompt: ChatMessage) -> StarterCodeData:
        response = await self.run_model(
            prompt, BaseInterviewPromptStrategy.RESPONSE_TYPE.STARTER_CODE_GENERATION
        )
        return response
