...
class TestSQLiteIntegration:
    ...
    async def test_complete_user_workflow(self, sqlite_db, sample_user_profile):
        ...
        await sqlite_db.add_json_data_output_to_database(
            user_id, session_id, "evaluation", test_data
        )

        # Retrieve JSON data
        retrieved_data = await sqlite_db.get_json_data_output_from_database(
            "evaluation", user_id, session_id
        )
...
class TestPostgreSQLIntegration:
    ...
    async def test_complete_user_workflow(self, postgresql_db, sample_user_profile):
        ...
        # Add JSON data
        test_data = {"score": 85, "feedback": "Good performance"}
        await postgresql_db.add_json_data_output_to_database(
            user_id, session_id, "evaluation", test_data
        )

        # Retrieve JSON data
        retrieved_data = await postgresql_db.get_json_data_output_from_database(
            "evaluation", user_id, session_id
        )
        assert retrieved_data is not None
        assert retrieved_data["score"] == 85
...
class TestCrossDatabase:
    ...
    async def test_data_consistency_across_databases(
        self, sample_user_profile, sample_simulation_config
    ):
    ...
    async def test_interface_compatibility(self, sqlite_db, sample_user_profile):
        ...
        required_methods = [
            ...
            "get_session_data",
            "update_session",
            "get_most_recent_session_id_by_user_id",
            "get_all_session_data",
            "add_dialog_to_database",
            "add_evaluation_output_to_database",
            "store_simulation_config",
            "get_simulation_config",
            "list_simulation_configs",
            "delete_simulation_config",
            "add_to_batch",
            "commit_batch",
            "add_json_data_output_to_database",
            "get_json_data_output_from_database",
        ]

        for method_name in required_methods:
            assert hasattr(sqlite_db, method_name), f"Method {method_name} not found"
            assert callable(getattr(sqlite_db, method_name)), (
