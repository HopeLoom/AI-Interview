...
from .base import DatabaseInterface, UserProfile, SessionData, CompanyProfile
...
class FirebaseAdapter(DatabaseInterface):
    ...
    async def get_session_data(self, user_id: str, session_id: str) -> Optional[SessionData]:
        """Get session data"""
        try:
            session_ref = self._firebase_db.db.collection("users").document(user_id).collection("sessions").document(session_id)
            doc = session_ref.get()
    ...
    async def get_image_url(self, user_id: str, file_name: str, cache_bust: bool = True) -> Optional[str]:
    ...
    async def upload_video(self, user_id: str, session_id: str, filename: str, content: bytes, content_type: str) -> str:
    ...
    async def get_activity_progress_analysis_output_from_database(self, user_id: str, session_id: str) -> Optional[Dict[str, Any]]:
        """Get activity progress analysis output"""
        return self._firebase_db.get_activity_progress_analysis_output_from_database(user_id, session_id)
    
    async def get_metadata_from_database(self, user_id: str, session_id: str) -> Optional[Dict[str, Any]]:
        """Get metadata from database"""
        return self._firebase_db.get_metadata_from_database(user_id, session_id)
    
    async def get_final_visualisation_report_from_database(self, user_id: str, session_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_candidate_evaluation_data(self, user_id: str, session_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    async def get_candidates_with_evaluations(self, company_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.log_error(f"Error getting candidates with evaluations for company {company_id}: {e}")
            return []
    ...
    async def get_interview_candidates(self, company_id: str, interview_id: str) -> List[Dict[str, Any]]:
        """Get candidates for a specific interview/job posting"""
        try:
            return await asyncio.to_thread(self._firebase_db.get_interview_candidates, company_id, interview_id)
        except Exception as e:
