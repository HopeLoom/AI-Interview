...
from evaluation_agent.base import BaseEvaluation, BaseEvaluationConfiguration, BaseEvaluationPromptStrategy, SubqueryGeneratorOutputMessage, SubqueryDataExtractionOutputMessage, SubqueryDataExtractionInputMessage, SubqueryGeneratorInputMessage
...
from master_agent.base import CommunicationMessage, SlaveMessageStructure, SimulationRole, InterviewRound, MasterMessageStructure, SystemMessageStructure, SystemMessageType
from core.resource.model_providers.schema import ChatMessage, AssistantChatMessage, ChatModelProvider
from core.prompting.prompt_strategies.evaluation_one_shot import EvaluationPromptStrategy, ChatPrompt
from core.database.base import DatabaseInterface
from logger_config import LoggerManager
from master_agent.base import QuestionCriteriaSpecificScoring, QuestionSpecificScoring
from master_agent.base import CriteriaSpecificScoring, OldEvaluationMessage, EvaluationInputMessage, QuestionSpecificEvaluationOutputMessage, SystemMessageStructure, InterviewDataMessageFromClient, SystemMessageType, WebSocketMessageTypeFromClient
from master_agent.base import TOPICS_HR_ROUND, TOPICS_TECHNICAL_ROUND, SUBTOPICS_TECHNICAL_ROUND, SUBTOPICS_HR_ROUND, EvaluationMessageToFrontEnd
...
from interview_details_agent.base import InterviewTopicData, CharacterDataOutput
...
from master_agent.base import CandidateEvaluationVisualisationReport, CriteriaScoreVisualSummary, CodeSubmissionVisualSummary
from master_agent.base import CodeDimensionSummary, CodeAnalysisVisualSummary, PanelistFeedbackVisualSummary, OverallVisualSummary
from master_agent.base import PanelistFeedbackVisualSummaryList, CriteriaScoreVisualSummaryList
...
class Evaluation(BaseEvaluation):
    ...
    async def _setup_interview_metadata(self):
        ...
        try:
            ...
            if interview_metadata:
                ...
                self.logger.info("Interview metadata not found. Setting interview completed to True")
        ...
        try:
            ...
            if image_path:
                ...
                await self.database.upload_image(
                    ...
                    f"{self.user_id}{self.IMAGE_EXTENSION}"
                )
            else:
    ...
    def get_panelist_profiles_for_current_interview_round(self) -> List[Profile]:
        ...
        if not hasattr(self, '_cached_panelist_profiles'):
            self._cached_panelist_profiles = [
                p.get_my_profile() for p in self.panelist_instances
    ...
    def parse_response_evaluation_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        evaluation_output:QuestionSpecificEvaluationOutputMessage = self.prompt_strategy.parse_response_evaluation_content(response)
        return evaluation_output
    
    def parse_response_subquery_generation_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        subquery_data:SubqueryGeneratorOutputMessage = self.prompt_strategy.parse_response_subquery_generation_content(response)
        return subquery_data
    
    def parse_response_subquery_data_extraction_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        subquery_data:SubqueryDataExtractionOutputMessage = self.prompt_strategy.parse_response_subquery_data_extraction_content(response)
        return subquery_data
    
    def parse_response_evaluation_summary_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        summary = self.prompt_strategy.parse_response_evaluation_summary_content(response)
        return summary
    
    def parse_response_code_analysis_visual_summary_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        code_analysis_visual_summary:CodeAnalysisVisualSummary = self.prompt_strategy.parse_response_code_analysis_visual_summary_content(response)
        return code_analysis_visual_summary
    
    def parse_response_overall_visual_summary_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        overall_visual_summary:OverallVisualSummary = self.prompt_strategy.parse_response_overall_visual_summary_content(response)
        return overall_visual_summary
    
    def parse_response_panelist_feedback_visual_summary_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        panelist_feedback_visual_summary:PanelistFeedbackVisualSummary = self.prompt_strategy.parse_response_panelist_feedback_visual_summary_content(response)
        return panelist_feedback_visual_summary
    
    def parse_response_criteria_visual_summary_content(self, response:AssistantChatMessage, prompt:ChatMessage):
        criteria_visual_summary:CriteriaScoreVisualSummary = self.prompt_strategy.parse_response_criteria_visual_summary_content(response)
    ...
    async def run_subquery_generator(self, topic_conversation) -> SubqueryGeneratorOutputMessage:
        ...
        try:
            ...
            prompt_input = PromptInput(
                ...
                activity_analysis=self.activity_instance.get_recent_progress() if self.activity_instance else None
            )
            ...
            await self.database.add_json_data_output_to_database(
                self.firebase_user_id, self.session_id, "subquery_data", subquery_data.model_dump() if hasattr(subquery_data, 'model_dump') else subquery_data
            )
    ...
    async def run_subquery_data_extraction(self, subqueries) -> SubqueryDataExtractionOutputMessage:
        ...
        try:
            ...
            await self.database.add_json_data_output_to_database(
                self.firebase_user_id, self.session_id, "subquery_data_extraction_output", subquery_data.model_dump() if hasattr(subquery_data, 'model_dump') else subquery_data
            )
    ...
    async def _evaluate_subtopic(self, topic_name: str, subtopic_name: str, topic_data):
        ...
        try:
            ...
            await self._perform_evaluation(
                evaluation_message, topic_conversation, subqueries_data, key, topic_name, subtopic_name, topic_data, subtopic_data
            )
    ...
    async def _process_subqueries(self, topic_name: str, topic_conversation) -> SubqueryDataExtractionOutputMessage:
    ...
    async def _perform_evaluation(self, evaluation_message, topic_conversation, subqueries_data, key, topic_name, subtopic_name, topic_data, subtopic_data):
        ...
        try:
            ...
            evaluation_output: QuestionSpecificEvaluationOutputMessage = await super().run_evaluation(prompt)
            ...
            # Save to database
            await self.database.add_json_data_output_to_database(
                self.firebase_user_id, self.session_id, "evaluation_output", evaluation_output.model_dump() if hasattr(evaluation_output, 'model_dump') else evaluation_output
            )

            # Evaluation from panelists
            await self.evaluation_from_panelists(topic_name, subtopic_name, topic_data, subtopic_data, topic_conversation, subqueries_data)
    ...
    async def evaluation_from_panelists(self, topic_name, subtopic_name, topic_data, subtopic_data, topic_conversation, subqueries_data):
    ...
    async def merge_evaluation_output(self):
        ...
        try:
            ...
            criteria_scoring_list, criteria_counter_list = self._initialize_criteria_scoring(metrics_covered)
            
            # Process evaluation outputs
            self._process_evaluation_outputs(criteria_scoring_list, criteria_counter_list, metrics_covered)
    ...
    def _get_metrics_covered(self) -> List[str]:
        ...
        metrics = self.interview_topic_tracker.get_metrics_covered_for_current_interview_round(InterviewRound.ROUND_TWO)
    ...
    def _process_evaluation_outputs(self, criteria_scoring_list, criteria_counter_list, metrics_covered):
        ...
        for evaluation_output in self.evaluation_topic_subtopic_mapping.values():
            for question_criteria_scoring in evaluation_output.question_criteria_specific_scoring:
                self._process_criteria_scoring(
                    question_criteria_scoring, criteria_scoring_list, criteria_counter_list, metrics_covered
                )

    def _process_criteria_scoring(self, question_criteria_scoring, criteria_scoring_list, criteria_counter_list, metrics_covered):
        ...
        for index, criteria in enumerate(criteria_scoring_list):
            if criteria_counter_list[index] > 0:
                if criteria.criteria.lower() == "coding":
                    criteria_scoring_list[index].score = 5 * round(criteria_scoring_list[index].score, 1)
                else:
                    criteria_scoring_list[index].score = 5 * round(
                        criteria_scoring_list[index].score / criteria_counter_list[index], 1
                    )
                
                self.logger.info(f"Criteria: {criteria.criteria}, Score: {criteria_scoring_list[index].score}")
                overall_score += criteria_scoring_list[index].score
                overall_analysis += criteria_scoring_list[index].reason + "\n" 
                counter += 1
    ...
    async def generate_evaluation_report(self) -> EvaluationMessageToFrontEnd:
        ...
        if self.is_interview_completed:
            ...
            conversation_history = self.interview_topic_tracker.get_conversation_history_for_all_topics(InterviewRound.ROUND_TWO)
            
            merged_evaluation_output, overall_analysis_summary, overall_score = await self.merge_evaluation_output()
            
            activity_code_from_candidate = self.activity_instance.get_activity_code_from_candidate() if self.activity_instance is not None else ""

            activity_analysis:ActivityProgressAnalysisSummaryForPanelistOutputMessage = self.activity_instance.get_recent_progress() if self.activity_instance is not None else ActivityProgressAnalysisSummaryForPanelistOutputMessage()
            ...
            evaluation_data.code_from_candidate = activity_code_from_candidate if activity_code_from_candidate is not None else ""
            ...
            panelist_names = []
            for panelist_instance in self.panelist_instances:
                if panelist_instance.my_profile.interview_round_part_of == InterviewRound.ROUND_TWO.value:
            ...
            evaluation_data.candidate_name = self.candidate_profile.background.name if self.candidate_profile is not None else ""
            evaluation_data.candidate_profile = self.candidate_profile.background if self.candidate_profile is not None else ""
            ...
            evaluation_data.activity_analysis = ActivityProgressAnalysisSummaryForPanelistOutputMessage()
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "final_evaluation_output", evaluation_data.model_dump() if hasattr(evaluation_data, 'model_dump') else evaluation_data)

        return evaluation_data
    

    async def run_code_analysis_visual_summary(self, code:str, progress_analysis:ActivityProgressAnalysisSummaryForPanelistOutputMessage):
        ...
        code_analysis_visual_summary:CodeAnalysisVisualSummary = await super().generate_code_analysis_visual_summary(prompt)
        return code_analysis_visual_summary


    async def run_overall_visual_summary(self, overall_analysis:str, overall_score:float) -> OverallVisualSummary:
        ...
        overall_visual_summary:OverallVisualSummary = await super().generate_overall_visual_summary(prompt)
        return overall_visual_summary


    async def run_panelist_feedback_visual_summary(self, panelist_feedback:List[str], panelist_names:List[str], panelist_occupations:List[str]):
        ...
        panelist_feedback_visual_summary:PanelistFeedbackVisualSummaryList = await super().generate_panelist_feedback_visual_summary(prompt)
        return panelist_feedback_visual_summary
    

    async def run_criteria_visual_summary(self, criteria_specific_scoring:List[CriteriaSpecificScoring]) -> CriteriaScoreVisualSummaryList:
        ...
        criteria_visual_summary:CriteriaScoreVisualSummaryList = await super().generate_criteria_visual_summary(prompt)
        return criteria_visual_summary

    async def revise_evaluation_report_for_visualization(self, evaluation_data:EvaluationMessageToFrontEnd) -> CandidateEvaluationVisualisationReport:
        self.logger.info("Revising evaluation report for visualization and converting to candidate visualization")
        ...
        criteria_specific_scoring_list:List[CriteriaSpecificScoring] = evaluation_data.evaluation_output.criteria_specific_scoring
        code_written_by_candidate:str = evaluation_data.code_from_candidate
        activity_analysis:ActivityProgressAnalysisSummaryForPanelistOutputMessage = evaluation_data.activity_analysis
        ...
        candidate_visualization_report:CandidateEvaluationVisualisationReport = CandidateEvaluationVisualisationReport()
        ...
        candidate_visualization_report.candidate_profile_image = evaluation_data.candidate_profile_image
        candidate_visualization_report.overall_score = evaluation_data.overall_score

        code_analysis_visual_summary:CodeAnalysisVisualSummary = await self.run_code_analysis_visual_summary(code_written_by_candidate, activity_analysis)
        self.logger.info(f"Code analysis visual summary: {code_analysis_visual_summary}")
        overall_visual_summary:OverallVisualSummary = await self.run_overall_visual_summary(overall_analysis, evaluation_data.overall_score)
        self.logger.info(f"Overall visual summary: {overall_visual_summary}")
        panelist_feedback_visual_summary:PanelistFeedbackVisualSummaryList = await self.run_panelist_feedback_visual_summary(panelist_feedback, evaluation_data.panelist_names, evaluation_data.panelist_occupations)
        self.logger.info(f"Panelist feedback visual summary: {panelist_feedback_visual_summary}")
        criteria_visual_summary:CriteriaScoreVisualSummaryList = await self.run_criteria_visual_summary(criteria_specific_scoring_list)
        ...
        candidate_visualization_report.panelist_feedback = panelist_feedback_visual_summary.panelist_feedback
        ...
        await self.database.add_json_data_output_to_database(self.firebase_user_id, self.session_id, "final_visualisation_report", candidate_visualization_report.model_dump() if hasattr(candidate_visualization_report, 'model_dump') else candidate_visualization_report)
    ...
    async def run(self):
        ...
        try:
            ...
            async with self.lock:
                ...
                evaluation_report:EvaluationMessageToFrontEnd = await self.generate_evaluation_report()
                report:CandidateEvaluationVisualisationReport = await self.revise_evaluation_report_for_visualization(evaluation_report)
